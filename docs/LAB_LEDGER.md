...
- P35 ROOT-CAUSE (L12 ×459, RESEARCH-459-L12.md): DDA-1 (21/350k=6.0e-5) = провал INLINE-CLIP-поверхности, НЕ марша — марш+corner с clip-VERBATIM = 0/200,736 (200k random + 736 adversarial: corner-exact/axis-edge/tMax-tie/large>2^24/stationary/sign-zero × 4 политики); hand-порт clip брикет [0..21.5%] (naive-slab F1-corner 160/320=50%, random 0/350k; Y-first random 75,341/350k). ПРАВИЛО: AABB.clip ВЕРБАТИМ только, inline-clip PARKED навсегда.
- P35-v2 ДИЗАЙН (числа): продакшн=ваниль walk (закон 5), shadow-транскрипция=тайт candidate-feed P31; верификатор 1/200 per-thread (n=750/тик): p=6e-5→4.4%/тик детект (медиана 15 тиков), P(не-детект/5мин)=1.9e-12, стоимость 0.18% MSPT@500ms; disarm-латч BATCH_OK one-shot (mismatch/threw/AIOOBE/NCDFE → фид off, per-pos serve4); экспозиция паритета 0 тиков (walk=истина). Фид-инкремент сверх P31 +1.19пп → пара база +22.7 с a26 +12.4@8671791; потолок лейна +14.1пп.
