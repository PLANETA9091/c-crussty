...
- P41 path-node neighbor cache (+0.6-1пп); P42 goal canUse sense-memo (+0.4-0.8); P43 brain flat-memory (+0.3-0.5); P45 navigatingMobs pre-gate (roaring, +0.3-0.6); P46 tick-deadband; P47 GoalSelector transition-diff iter-3.
- ×459-L08 javap-глубина (kernel jar round-396-a): MoveControl.tick 585 бк + PathNavigation.tick 185 бк + PathFinder.findPath A* сняты; константы вербатим: d2-гейт 2.500000277905201E-7, yRot-π = (double)(float)Math.PI = 3.1415927410125732 (f32-π в f64-делителе!), 0.017453292f; ТРАПЫ бит-в-байт: Mth.sin/cos = SIN[65536]-таблица (idx=(int)(x*10430.378f)&0xFFFF, cos:+16384.0f) НЕ IEEE; Mth.atan2 = fastInvSqrt (магия 6910469410427058090L, 1 Newton-шаг) + ASIN_TAB[257]/COS_TAB[257] (asin(i/256.0), FRAC_BIAS=longBitsToDouble(4805340802404319232L)) НЕ Math.atan2; MoveControl.tick CPU-self <0.6% (ниже top-40 cutoff) — лейн в плотности вызовов, не в арифметике.
- P44 MoveControl navmath bulk-JNI (+0.8-1.2, javap-транскрипция IEEE754 канон; ×459-L08 уточнение: ядро +0.5-0.9пп при срезе 60-70% × захват 30-40%, семейство P41-P47 +0.83-1.28пп, потолок лейна +3.2пп — композиционный кирпич, не самостоятельный бар); дизайн MovePlaneOps.moveDecide готов (RESEARCH-459-L08 §4): коллект-плоские массивы → 1 JNI → apply setYRot/setSpeed/jump, ERR-ladder batchOk→decideJava, java-хвост getBlockState/shape остаётся ванилью; Purpur MoveControllerWASD.tick→vanillaTick→MoveControl.tick = единая точка ретаргета; Drowned/Guardian/SmoothSwimming vanilla-оверрайды вне хука (0.015% wall, fail-safe).
