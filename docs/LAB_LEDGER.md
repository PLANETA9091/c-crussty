...
- ROOT-CAUSE roar-2 −10.7@8960455 (L05 ×459, RESEARCH-459-L05.md): НЕ спящий блоб — gate lever_matches ПРОЙДЕН (нет dormant-печати), probe OK, natives собраны CI из рефа; упало на seedAll()==0 → stdout:947 «seed failed (0)» → hook dormant → 0 roar/bloom-фреймов (cpu 112915/wall 61212/alloc сэмплов). seed(0) = глотаемый Throwable в java-цикле или eidxFlush ERR_STRUCT; ёмкости 8192/шард vs ~280/шард — не переполнение. Фикс = инструментировать rc/Throwable + re-seed после инжекта; jar байт-в-байт равен базе (ops-классы инжектит агент — «плоский jar» ≠ спящий гейт). RED −10.7 = STW-кластер co-рана: Full=10 (банк 7), STW 23.55s vs chk-14 17.66s, Full-хвост 2249/2277ms; norm↔STW r≈−0.93 по 5 ногам. ЛОЖНЫЙ ARM-маркер: absorb дал {"cmp457_paldelta": "MARKER"} из dormant-подсказки — парсер матчил подстроку lever-id; анкерить на полный ARMED-маркер. Capture: broadphase 9.90% presence/5.06% self → потолок +12.5пп, Δ-прогноз +4..+7пп → суб-бар, компоновка с P31 обязательна.
