...
- L11 WILD-лаб ×459 (H05 papaya-epoch-ридеры, RESEARCH-ONLY): лейн chk-14 9.36% (9647/103062; 16:9.87%, 11:8.82%, anchor33:14.68%), reader-core 4.0% CPU (getEntities 1120 + getHardColliding 926 + intersects 871 + CHM/VarHandle доли ≈820), voxel-хвост 57% — не ридеры. javap: plain count/size/storage (stale+shift-left JMM-риск), checkThread-throw + ReentrantAreaLock$Node 0.8% alloc-событий = формальные блокеры off-thread чтения; ваниль-порядок выдачи (LongSortedSet, minY−4.0) ≠ moonrise (z-outer/x-inner, y-asc, −2.0/+2.0) → паритет против КЕРНЕЛА. Офлайн JDK-21 ×3: lockstep-паритет 0/300 ×3 PASS (порядок z,x,y-asc,idx воспроизведён epoch-снапшотами), race-demo unlock 0/400 INCONCLUSIVE (JMM-окно узко — не репрод), fail-closed PASS (fallbacks 80% при rebuild-шторме 1/8 — в проде per-tick-phase), speedup 0.73-1.35× медиана 1.21×. CAPTURE: 9.36% × 8-15% → Δ +0.8..+1.4пп, потолок +9.36пп (реалистичный +4.1пп) — swing-слой климба chk-14 (дефицит +10.7пп), НЕ pair-maker сам по себе; комбо P31⊕P32/P36⊕papaya → нога 29.0-33.6 → пара с a26 = 16.6-21.2. Гейт-канон: PapayaReadOps EARLY-define + fallbacks<1% + лейн-проверка иначе REFUTED (гейты G1-G6 в RESEARCH-459-L11.md).
