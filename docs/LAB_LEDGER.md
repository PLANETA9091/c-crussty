...
- ×459-L01 capture-матем P31: лейн chk-14 15.36% (15830 сэмплов; депресс 16.30-16.63 абс.+41-43% vs якорь 12093) = discovery 3.3пп + visit 5.9пп + carrier-overhead 5.3пп (serve4 2303/gate 1125/CHM.get 992/LongSet.add 999/MH-wrapper 96.5% стеков) захватываемо 14.5пп; strict java tail ~1пп (applyEffectsFromBlocks 859+StepBasedCollector) не захватывается. Рост лейна идёт в visit/discovery ядро, не в мемо-хвост → гипотеза B подтверждена (secWrite→CHM<Section,Snap>-инвалидации→serve4 MISS-домино; java_util 7.01→8.7-8.96 синхронно). javap 7: checkInsideBlocks бюджет 16/axis+visitedBlocks LongSet; toAabbs() аллокация на каждый не-air блок (Lithium-цель); InsideBlockOps flat SLOT/VIS/EFF = 70% P32; CollideBatchOps buildPlan/BiPredicate = канон P31. Δ(P31@chk-14)=+7.8..+12.1пп (база +10.2), потолок +14.1пп (94% лейна); нога 33.9 база; пара с a26 +12.4@8671791 = +21.5 ✓ (консерватив +19.1 → якорь ≤+11.5). Гейты G1-G6 preregistered (RESEARCH-459-L01.md §6); закон-5 вектора НЕ активировать.
...
- **ЗАКРЫТИЕ H03-пары ×459-L04 (capture-матем, RESEARCH-459-L04)**: Hilbert-порядок забирает ТОЛЬКО энумерацию leg-1 (EntityCollectionBySection.getEntities 0.93% + getHardCollidingEntities 0.77% = лейн 1.70пп от 104163 сэмплов); AABB.intersects 0.70% выживает в strict-хвосте (javap: intersects(DDDDDD)=6×dcmpg из арена-хвоста). Δ-прогноз ноги = 2.6 ⊕ 0.7-1.0 = +3.3-3.6пп, АБСОЛЮТНЫЙ потолок +4.3пп. Пара ≥+20 требует якорь ≤−16.4..−15.7: в банке ×459 (12) только poi-14 −18.1@7223047 (1/12=8.3%); a33 −11.8 даёт max +16.1<20; медианная пара пула ≈+5.7-6.0пп; лотерея poi-14 P≈0.24%/ролл, EV 0.07 при ≥30 роллах, min-of-3 EV≈1e-4 → **КОМПОЗИЦИЯ <20пп, ЗАКРЫТА (закон 13a)**. H07/Hilbert = микроридер +0.7-1.0пп (cargo-ready @round-458j-hilbert), не пара-анлокер; живые ≥+20 пути = окна B/E монстров.
