...
| G-COMBO (extension probe, non-gating) | combined arming of BOTH proven bridges (PerlinNoise + improved_noise 5-arg) adds a measurable win on top of G-AB | one bench session, TASK-74 harness verbatim + dual env differential | **NO-GO (2026-09-08, TASK-79): n=5/arm ABBA, cpu_burst −0.70 CPU-s median (−1.2%) p_two=0.8413, wall −1.58 s (−2.3%) p_two=0.6905 — indistinguishable from zero; both B-arm markers verified armed every run; world byte-identical; 0 new hs_err. Mechanism-consistent: ImprovedNoise.noise(DDDDD)D is a SMALL body that inlines cleanly into callers (BlendedNoise octave loops) — swapping it trades zero-transition inlined calls for JNI transitions (35–90 ns floor); whole-swap ROI is inversely related to body inlineability (11KB barrier body = −11.1% live; small bodies ≈ wash). G-AB's JFR `p(int)` 10.43% residue was attribution of INLINED code in caller frames, not addressable Java body cost. The noise channel is thereby measured shut on all fronts: PerlinNoise GO / NormalNoise decode-GO-but-parked (−0.1%/burst) / BlendedNoise kernel absent / COMBO NO-GO — `bench/e2e/results/COMBO_AB_2026-09-09.md` |
