...
print(f"{6000000/(time.time()-t):.0f}")' 2>/dev/null || echo unknown)"
{
  ...
  echo "gc_tune: $GC_TUNE (GC-TUNE TASK-375/376/380/384 + S99-gcw ROUND-468-S15; 1 = MaxGCPauseMillis=40 + IHOP=35 + G1HeapRegionSize=8m + AlwaysPreTouch; 2 = IHOP=35 + 8m + AlwaysPreTouch без pause-target [s7199: pause-target токсичен]; 3 = COLLECTOR ParallelGC [БАНК v4]; 4 = COLLECTOR ZGC generational; 5 = ParallelGC + TransparentHugePages + AlwaysPreTouch — JVM-level, vanilla-parity; 6 = ParallelGC + MetaspaceSize=256M + ReservedCodeCacheSize=512M — threshold-cascade kill, javap-neutral)"
...
if [ "${GC_TUNE:-0}" = "1" ]; then
  ...
  log "gc_tune=6: ParallelGC + MetaspaceSize=256M + ReservedCodeCacheSize=512M (S99-gcw/ROUND-468-S15: Full=10 = 8x threshold-cascade [4x Metadata GC Threshold 741ms + 6x CodeCache GC Threshold ~11s], 0 от старого гена — НЕ аллокация; metaspace-пик 171.7MB < 256M => MD-Full=0 детерминированно, RCC x2.13 => CC-Full 6->2-3; HOST STW 27.5->16.8-20.3s; young не тронут, javap-neutral 0 Java)"
fi
