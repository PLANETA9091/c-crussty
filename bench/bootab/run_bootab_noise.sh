#!/usr/bin/env bash
# bench/bootab/run_bootab_noise.sh — TASK-58 derived harness (agent-7625532f, 2026-09-09).
# Derived from run_bootab.sh (byte-identical EXCEPT server.properties): the TASK-57 JFR
# profile measured the boot noise burst (structure-ring gen, 2.87 CPU-s) which does NOT
# exist in the canonical flat/no-structures throwaway world — this variant measures the
# arm delta on a world where that stack actually runs. Props delta:
#   level-type=minecraft:normal, generate-structures=true, view-distance=2,
#   level-seed=90919058 (FIXED — deterministic worldgen workload per run, paired-A/B
#   discipline same as P500 fixed inputs). Everything else (assembly, markers, kill,
#   TSV) unchanged. See bench/bootab/results/BOOTAB_NOISE_2026-09-09.md.
...
for i in $(seq 1 "$RUNS"); do
  ...
  printf 'eula=true\n' > "$rundir/eula.txt"
  {
    ...
    printf 'level-type=minecraft\\:normal\n'
    printf 'generate-structures=true\n'
    printf 'view-distance=2\n'
    printf "simulation-distance=4\n"
    printf "level-seed=90919058\n"
