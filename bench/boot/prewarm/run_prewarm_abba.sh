#!/usr/bin/env bash
# TASK-97 (S7-40): parallel <clinit> prewarm spike — within-session ABBA.
...
boot_prewarm() { # $1 = A|B
    ...
    if [ "$1" = A ]; then AGOPT=""; else AGOPT="=$LST"; fi
...
-jar '$SERVER/versions/purpur-1.21.10.jar' --nogui" </dev/null >"$OUT/arm_${1}_$$.log" 2>&1 &
    ...
    PW=$(grep -m1 -oE 'worker[0-9] done ok=[0-9]+ fail=[0-9]+' "$OUT/arm_${1}_$$.log")
