...
## ADDENDUM 2026-09-07T18:15Z — TASK-30 hygiene applied (agent-7625532f)

Per TASK-30's bench-hygiene recommendation, `AreaMapBench.java` now uses FIXED
iteration windows (identical RNG stream in FAKE and REAL), resets the radius
walk to the canonical state per size, and clamps it to [d-8, d+8] (clamp
applied BEFORE the same-state force check — a post-check clamp silently
produced skipped-native calls, caught by the verification and fixed).
Cross-mode ops/call is now identical by construction: 620 / 2506 / 5056 at
d=63/255/511 in BOTH modes.

Corrected rerun (both modes VERIFICATIONS PASS; box busier than the first
run — sibling soak active — treat ratios, not absolutes):
- FAKE CHANGED/NAIVE: 1.10x / 1.03x / 1.03x (was 2.31/1.21/1.08 with walk
  drift) — pipeline overhead is ~3-10%, the first run's small-grid 2.31x was
  drift-inflated.
- REAL CHANGED/NAIVE: 2.34x / 5.22x / 6.05x — consistent with the first run
  (4.48/6.44/6.18); the closed kernel remains the cost center.
- SAME fast path 0.4-49 ns/call, 0 native, 0 callbacks (JIT elision variance
  across runs; steady-load figure ~25 ns).
Raw TSV: results/apply_bench_raw.tsv (regenerated by this rerun).
