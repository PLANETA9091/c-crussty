#!/usr/bin/env bash
# TASK-106 — JFR warm-burst mechanism diff: ARMED vs DORMANT (agent-7625532f)
...
OUT="$PWD/bench/graal_ab/RAW_TASK106"; mkdir -p "$OUT"
...
echo "start-TASK106-cold-$STAMP" >> /home/z/BENCH.lock.journal
trap 'echo "done-TASK106-cold-$STAMP" >> /home/z/BENCH.lock.journal' EXIT
...
run_one() { # $1=ARM $2=idx
    ...
    # COLD protocol: NO warm forceload — noise classes + JIT compile inside the measured burst
    local TW="cold(none)"
    # COLD regime: arming happens DURING the burst by design -> POST-HOC verification after burst
    local ARM_NOTE="dormant-cold"
    ...
    if [ "$ARMED" = 1 ]; then
        ...
        if [ "$G1" = 1 ] && [ "$G2" = 1 ] && [ "$G3" = 1 ]; then
            ARM_NOTE="armed-posthoc-verified"
        else
            log "INVALID: $RID post-hoc arming evidence ABSENT — LOUD ABORT (cold run never armed)"
...
RANGE="${RUNS:-1-${#SEQ[@]}}"
I0=${RANGE%%-*}; I1=${RANGE##*-}
log "TASK-106 JFR diff begin: ${#SEQ[@]} runs total, executing $I0..$I1"
for ((i=I0; i<=I1; i++)); do
    ARM=${SEQ[$((i-1))]}
    run_one "$ARM" "$i" || true
done
...
log "TASK-106 JFR diff range $I0..$I1 complete"
