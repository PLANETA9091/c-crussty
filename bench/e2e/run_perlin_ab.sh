...
# env CRUSSTY_NATIVE_PERLIN_NOISE=1 (whole-body getValue swap) vs production
...
#         B CRUSSTY_NATIVE_PERLIN_NOISE=1
...
RAW=$RES/PERLIN_AB_RAW_2026-09-09
SEED_TAR=$SERVER_DIR/world_ab_seed_task74.tar.gz
N_PER_ARM=5
...
boot_arm() { # $1 = A|B ; boots via canonical orchestrate, stdout to RAW
    local arm=$1 out=$2
    if [ "$arm" = B ]; then
        CRUSSTY_NATIVE_PERLIN_NOISE=1 "$E2E" boot >"$out" 2>&1
    else
        env -u CRUSSTY_NATIVE_PERLIN_NOISE "$E2E" boot >"$out" 2>&1
    fi
}
...
armed_markers() { # $1 = A|B -> rc 0 iff arm state as expected (loud on surprise)
    local arm=$1
    if [ "$arm" = B ]; then
        grep -qE 'perlin_noise: hook armed|perlin_noise: self-test passed' "$E2E_LOG"
    else
        if grep -qE 'perlin_noise: hook armed|perlin_noise: self-test passed' "$E2E_LOG"; then return 1; fi
        return 0
    fi
}
...
run_one() { # $1 = idx, $2 = A|B
    ...
    if [ "$arm" = B ]; then
        ...
        while ! grep -qE 'perlin_noise: hook armed|perlin_noise: self-test passed' "$E2E_LOG" && [ "$armwait" -lt 90 ]; do
            sleep 1; armwait=$((armwait + 1))
        done
        if ! grep -qE 'perlin_noise: hook armed|perlin_noise: self-test passed' "$E2E_LOG"; then
        ...
        grep -qE 'perlin_noise: hook armed|perlin_noise: self-test passed' "$E2E_LOG" && armv=UNEXPECTED_ARM
    fi
...
run_jfr_proof() { # non-timed arm-B run with JFR (mechanism proof, TASK-57 recipe)
    local jfrdir=$LOGS_DIR/jfr_task74
    mkdir -p "$jfrdir"
    ...
    JAVA_TOOL_OPTIONS="-XX:StartFlightRecording=filename=$jfrdir/server_%p.jfr,dumponexit=true,settings=profile,maxsize=128M" \
        CRUSSTY_NATIVE_PERLIN_NOISE=1 "$E2E" boot >"$RAW/jfrproof_boot.txt" 2>&1 \
    ...
    sleep 5
    grep -E 'perlin_noise: hook armed|perlin_noise: self-test passed|perlin_noise: computed patch|perlin_noise: hook serve' "$E2E_LOG" >"$RAW/jfrproof_markers.txt" 2>/dev/null || true
    ...
    sleep 40  # mid-burst
    "/home/z/jdk21/bin/jcmd" "$pid" JFR.dump filename="$jfrdir/task74_b_midburst.jfr" \
...
main() {
    ...
    local ref=$SERVER_DIR/.world_ab_ref_task74
