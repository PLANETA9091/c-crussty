#!/usr/bin/env bash
# TASK-74 (agent-7625532f, 2026-09-09): G-AB decisive paired A/B — whole-object
# PerlinNoise bridge (TASK-73 Session-1, whole-body swap, derived from TASK-70 ABI).
# First real-load A/B of the batching-layer bridge. Single differential:
# env CRUSSTY_FLUID_PUSH_GUARD=1 (whole-body getValue swap) vs production
# default; improved_noise stays dormant in BOTH arms (cleaner than TASK-63).
#
...
RAW=$RES/FLUID_AB_RAW_2026-09-08
SEED_TAR=$SERVER_DIR/world_ab_seed_task80.tar.gz
N_PER_ARM=5
...
boot_arm() { # $1 = A|B ; boots via canonical orchestrate, stdout to RAW
    local arm=$1 out=$2
    if [ "$arm" = B ]; then
        CRUSSTY_FLUID_PUSH_GUARD=1 "$E2E" boot >"$out" 2>&1
    else
        env -u CRUSSTY_FLUID_PUSH_GUARD "$E2E" boot >"$out" 2>&1
    fi
}
...
armed_markers() { # $1 = A|B -> rc 0 iff arm state as expected (loud on surprise)
    local arm=$1
    if [ "$arm" = B ]; then
        grep -qE 'fluid_guard: hook armed|fluid_guard: self-test passed' "$E2E_LOG"
    else
        if grep -qE 'fluid_guard: hook armed|fluid_guard: self-test passed' "$E2E_LOG"; then return 1; fi
        return 0
    fi
}

# TASK-74 protocol fix (tail probe 2026-09-09): post-Done async work decays
...
run_one() { # $1 = idx, $2 = A|B
    ...
    if [ "$arm" = B ]; then
        ...
        while ! grep -qE 'fluid_guard: hook armed|fluid_guard: self-test passed' "$E2E_LOG" && [ "$armwait" -lt 90 ]; do
            sleep 1; armwait=$((armwait + 1))
        done
        if ! grep -qE 'fluid_guard: hook armed|fluid_guard: self-test passed' "$E2E_LOG"; then
            log "$tag NO_ARM after ${armwait}s — INVALID B-run, aborting (shutdown+restore)"
            grep -E 'crussty|fluid_guard' "$E2E_LOG" >"$RAW/${tag}_markers.txt" 2>/dev/null
        ...
        grep -qE 'fluid_guard: hook armed|fluid_guard: self-test passed' "$E2E_LOG" && armv=UNEXPECTED_ARM
    fi
    # per-run marker evidence (fresh E2E_LOG = this boot only)
    grep -E 'crussty|fluid_guard|hook|self-test|kernel' "$E2E_LOG" \
        >"$RAW/${tag}_markers.txt" 2>/dev/null
    log "$tag arm-validation=$armv wait=${armwait}s done=${done_s}s pid=$pid"
    # ---- SYMMETRIC idle gate (TASK-74): identical pre-burst state both arms
    ...
    # Sep 7 — world identity proven directly), so the threshold moves to 32 s.
    # World identity is additionally anchored by the seed tar created from
    # the verified-intact world (region inventory check in session log).
    if [ -n "${done_s:-}" ] && awk -v d="$done_s" 'BEGIN{exit !(d>32)}'; then
        log "FATAL: $tag Done=${done_s}s > 32s — fresh-world symptom (INCIDENT guard)"
    ...
    # ---- T0: entity-load profile (TASK-78 census shape, deterministic).
    # Fluid-push is hot under ENTITY load, not worldgen. Summon a fixed 20x20
    # grid of 400 stone items (deterministic coordinates — NO RNG: both arms
    ...
    for ((i = 0; i < 400; i++)); do
        ...
        fifo_send "summon minecraft:item $sx 76.0 $sz {Item:{id:\"minecraft:stone\",count:1},Age:-32768s,PickupDelay:-1s}" \
            || log "$tag summon $i rc=$?"
    done
    local summoned
    ...
    # ---- FIXED measurement window: 30 s of steady-state item ticking
    cpu0=$(cpu_ticks "$pid"); t0=$(date +%s.%N)
    local trace="$RAW/${tag}_cputrace.txt"; : >"$trace"
    local tprev=$t0 cpuprev=$cpu0 now c dt cores elapsed
    while :; do
        sleep 0.5
        now=$(date +%s.%N)
        c=$(cpu_ticks "$pid")
        if [ -z "$c" ]; then log "$tag child vanished mid-window"; break; fi
        dt=$(awk -v a="$now" -v b="$tprev" 'BEGIN{print a-b}')
        cores=$(awk -v x=$((c - cpuprev)) -v d="$dt" 'BEGIN{print x/(d*100)}')
        echo "$now $cores" >>"$trace"
        tprev=$now; cpuprev=$c
        elapsed=$(awk -v a="$now" -v b="$t0" 'BEGIN{print a-b}')
        if awk -v e="$elapsed" 'BEGIN{exit !(e>=30)}'; then break; fi
    done
    local tend=$(date +%s.%N); local cpuend=$(cpu_ticks "$pid")
    ...
    # ---- teardown: items + forceload, then relay-alive proof
    fifo_send "kill @e[type=item]" || true
    sleep 2
    local qout="$RAW/${tag}_kill_count.txt"
    tail -c 2000 "$LATEST" >"$qout" 2>/dev/null
...
run_jfr_proof() { # non-timed arm-B run with JFR (mechanism proof, TASK-57 recipe)
    local jfrdir=$LOGS_DIR/jfr_task80
    mkdir -p "$jfrdir"
    ...
    JAVA_TOOL_OPTIONS="-XX:StartFlightRecording=filename=$jfrdir/server_%p.jfr,dumponexit=true,settings=profile,maxsize=128M" \
        CRUSSTY_FLUID_PUSH_GUARD=1 "$E2E" boot >"$RAW/jfrproof_boot.txt" 2>&1 \
        || { log "JFR-proof BOOT FAIL"; return 1; }
    local pid; pid=$(server_pid)
    # TASK-74 fix: the arming chain (force-load retries + bridge define +
    ...
    local jfrwait=0
    while ! grep -qE 'fluid_guard: self-test passed' "$E2E_LOG" && [ "$jfrwait" -lt 150 ]; do
        sleep 1; jfrwait=$((jfrwait + 1))
    done
    log "JFR proof arming wait=${jfrwait}s (armed=$([ "$jfrwait" -lt 150 ] && echo yes || echo NO))"
    sleep 5
    grep -E 'fluid_guard: hook armed|fluid_guard: self-test passed|fluid_guard: computed patch|fluid_guard: hook serve' "$E2E_LOG" >"$RAW/jfrproof_markers.txt" 2>/dev/null || true
    ...
    sleep 12
    log "JFR proof: item load live, mid-window dump in 18 s"
    sleep 18  # mid-window
    "/home/z/jdk21/bin/jcmd" "$pid" JFR.dump filename="$jfrdir/task80_b_midburst.jfr" \
    ...
    sleep 2
    # TASK-74 teardown hardening: the JFR boots orphaned the kernel JVM twice
...
main() {
    ...
    local ref=$SERVER_DIR/.world_ab_ref_task80
