#!/usr/bin/env bash
# TASK-79 (agent-7625532f, 2026-09-09): G-COMB0 paired A/B — COMBINED arming of
# the two proven noise bridges: whole-object PerlinNoise (TASK-73/74, -11.1%
# cpu_burst live) + ImprovedNoise 5-arg noise (improved_noise.rs, P500-proven).
# Rationale (armed JFR census, task74_b_midburst.jfr): after the PerlinNoise
# bridge, the top Java leaf is ImprovedNoise.p(int) 10.43% + noise 2.05% — the
# octave-gradient math still runs in Java because improved_noise stayed dormant
# in the TASK-74 arms (env-gated). Combined arming covers it through the SAME
# proven native kernels. Single differential:
#   B: CRUSSTY_NATIVE_PERLIN_NOISE=1 + CRUSSTY_NATIVE_IMPROVED_NOISE=1
# vs production default (A: both unset).
#
# Derived harness — scripts/e2e_orchestrate.sh is used UNTOUCHED (boot/shutdown).
#   SAME deployed module .so in both arms; the differential is env only.
...
#   boot -> Done + armed-marker validation -> SYMMETRIC idle gate (CPU < 0.12
#   cores for 3 x 0.5 s; post-Done tail probe: decay reaches floor ~55 s)
#   -> settle
...
RAW=$RES/COMBO_AB_RAW_2026-09-09
SEED_TAR=$SERVER_DIR/world_ab_seed_task79.tar.gz
N_PER_ARM=5
...
IDLE_STREAK=6     # consecutive 0.5 s samples below IDLE_RATE = burst over (v2: 3 was bursty-gap-vulnerable)
...
boot_arm() { # $1 = A|B ; boots via canonical orchestrate, stdout to RAW
    local arm=$1 out=$2
    if [ "$arm" = B ]; then
        CRUSSTY_NATIVE_PERLIN_NOISE=1 CRUSSTY_NATIVE_IMPROVED_NOISE=1 "$E2E" boot >"$out" 2>&1
    else
        env -u CRUSSTY_NATIVE_PERLIN_NOISE -u CRUSSTY_NATIVE_IMPROVED_NOISE "$E2E" boot >"$out" 2>&1
    fi
}
...
armed_markers() { # $1 = A|B -> rc 0 iff arm state as expected (loud on surprise)
    local arm=$1
    if [ "$arm" = B ]; then
        grep -qE 'perlin_noise: hook armed|perlin_noise: self-test passed|improved_noise: hook armed|improved_noise: self-test' "$E2E_LOG"
    else
        if grep -qE 'perlin_noise: hook armed|perlin_noise: self-test passed|improved_noise: hook armed|improved_noise: self-test' "$E2E_LOG"; then return 1; fi
        return 0
    fi
}

# TASK-79 protocol fix (tail probe 2026-09-09): post-Done async work decays
# SLOWLY (1.0 cores @2s -> 0.15 @28s -> floor 0.090 @55s). Without a symmetric
# gate the A arm (10 s post-Done) swallows ~5 CPU-s of boot tail into its burst
# while the B arm (15-64 s arming wait) does not — inflating any B advantage.
# Both arms therefore wait for the SAME idle state before the burst starts.
wait_idle() { # $1 = pid; >=60 s dwell, then CPU < 0.12 cores sustained 5 s (cap 180 s)
    # v2: bursty decay gaps defeat a short streak (run pilot showed the gate
    # passing 2 s in while the tail probe proved work until ~55 s) — so the
    # gate now enforces a 60 s MINIMUM dwell (tail probe: floor ~0.090 at 55 s)
    # and 10 consecutive samples (5 s) below 0.12 cores. Both arms start every
    # burst at the SAME idle-floor state; slow-arming B runs dwell even longer,
    # which is harmless once the floor is reached.
    local pid=$1 prev=$(cpu_ticks "$pid") tprev=$(date +%s.%N) streak=0
    local waited=0 now c dt cores
    while [ "$waited" -lt 360 ]; do
        ...
        cores=$(awk -v x=$((c - prev)) -v d="$dt" 'BEGIN{print x/(d*100)}')
        tprev=$now; prev=$c
        if [ "$waited" -ge 120 ] && awk -v r="$cores" 'BEGIN{exit !(r<0.12)}'; then
            streak=$((streak + 1))
    ...
    log "wait_idle: cap reached — proceeding (state logged)"
    return 0
}
...
run_one() { # $1 = idx, $2 = A|B
    ...
    if [ "$arm" = B ]; then
        ...
        while ! grep -qE 'perlin_noise: hook armed|perlin_noise: self-test passed|improved_noise: hook armed|improved_noise: self-test' "$E2E_LOG" && [ "$armwait" -lt 90 ]; do
            sleep 1; armwait=$((armwait + 1))
        done
        if ! grep -qE 'perlin_noise: hook armed|perlin_noise: self-test passed|improved_noise: hook armed|improved_noise: self-test' "$E2E_LOG"; then
            log "$tag NO_ARM after ${armwait}s — INVALID B-run, aborting (shutdown+restore)"
            grep -E 'crussty|perlin_noise' "$E2E_LOG" >"$RAW/${tag}_markers.txt" 2>/dev/null
        ...
        grep -qE 'perlin_noise: hook armed|perlin_noise: self-test passed|improved_noise: hook armed|improved_noise: self-test' "$E2E_LOG" && armv=UNEXPECTED_ARM
    fi
    # per-run marker evidence (fresh E2E_LOG = this boot only)
    grep -E 'crussty|perlin_noise|hook|self-test|kernel' "$E2E_LOG" \
        >"$RAW/${tag}_markers.txt" 2>/dev/null
    log "$tag arm-validation=$armv wait=${armwait}s done=${done_s}s pid=$pid"
    # ---- SYMMETRIC idle gate (TASK-79): identical pre-burst state both arms
    local idle_rc; wait_idle "$pid"; idle_rc=$?
    log "$tag idle-gate rc=$idle_rc"
    # Canary: TASK-63 era threshold was 22 s (fresh world ~27 s vs pregen
    # 16-18 s). With the TASK-73 .so the SAME pre-generated world boots at
    # 25.4-29.7 s (verify boot measured 25.4 s; region mtimes untouched since
    # Sep 7 — world identity proven directly), so the threshold moves to 40 s
    # for the combo arm (TWO bridge modules arm during boot — two retransforms
    # + two self-tests; TASK-74 measured +3-4 s boot drift for ONE module, the
    # second arming adds a similar increment; world identity remains anchored
    # by the seed tar + region inventory).
    if [ -n "${done_s:-}" ] && awk -v d="$done_s" 'BEGIN{exit !(d>40)}'; then
        log "FATAL: $tag Done=${done_s}s > 40s — fresh-world symptom (INCIDENT guard)"
...
run_jfr_proof() { # non-timed arm-B run with JFR (mechanism proof, TASK-57 recipe)
    local jfrdir=$LOGS_DIR/jfr_task79
    mkdir -p "$jfrdir"
    ...
    JAVA_TOOL_OPTIONS="-XX:StartFlightRecording=filename=$jfrdir/server_%p.jfr,dumponexit=true,settings=profile,maxsize=128M" \
        CRUSSTY_NATIVE_PERLIN_NOISE=1 CRUSSTY_NATIVE_IMPROVED_NOISE=1 "$E2E" boot >"$RAW/jfrproof_boot.txt" 2>&1 \
        || { log "JFR-proof BOOT FAIL"; return 1; }
    local pid; pid=$(server_pid)
    # TASK-79 fix: the arming chain (force-load retries + bridge define +
    # retransform + selftest) can take 15-80+ s post-Done — the old sleep 6
    # started the burst BEFORE retransform, so the dump captured the pure-Java
    # profile (honest but useless as engagement proof). Wait for the armed
    # markers like run_one does, then give the bridge 5 s of settle.
    local jfrwait=0
    while ! grep -qE 'perlin_noise: self-test passed' "$E2E_LOG" && [ "$jfrwait" -lt 150 ]; do
        sleep 1; jfrwait=$((jfrwait + 1))
    done
    log "JFR proof arming wait=${jfrwait}s (armed=$([ "$jfrwait" -lt 150 ] && echo yes || echo NO))"
    sleep 5
    grep -E 'perlin_noise: hook armed|perlin_noise: self-test passed|improved_noise: hook armed|improved_noise: self-test|perlin_noise: computed patch|perlin_noise: hook serve' "$E2E_LOG" >"$RAW/jfrproof_markers.txt" 2>/dev/null || true
    ...
    sleep 40  # mid-burst
    "/home/z/jdk21/bin/jcmd" "$pid" JFR.dump filename="$jfrdir/task79_b_midburst.jfr" \
    ...
    sleep 2
    # TASK-79 teardown hardening: the JFR boots orphaned the kernel JVM twice
    # (launcher killed, kernel child survived) — an orphan autosaves onto the
    # restored world and poisons the next session. Wait out the kernel
    # explicitly, escalate to kill, and only then restore.
    local orphan=0 kp
    for kp in $(pgrep -f 'agentpath:[^ ]*libcrussty_runtime\.so'); do
        orphan=1
        kill "$kp" 2>/dev/null
    done
    if [ "$orphan" = 1 ]; then
        sleep 5
        pgrep -f 'agentpath:[^ ]*libcrussty_runtime\.so' | xargs -r kill -9 2>/dev/null
        sleep 2
        log "JFR teardown: orphan kernel JVM reaped"
    fi
    world_restore
...
main() {
    ...
    local ref=$SERVER_DIR/.world_ab_ref_task79
