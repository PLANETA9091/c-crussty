
/** P500 bench group 17 — generated. DO NOT EDIT.
 *  PaperNativeJigsawCanAttach(I[I[I[I[I[Z[I[I[J)I -> [oldBatchSummary, optimizedBatchSummary, targetFirstBatchSummary]
 *  Typed argument fields + direct static calls into the kernels:
 *  zero reflection in the hot loop, zero boxing of kernel results. */
public final class G17 implements p500.Group {
    private static final int[] SMALL = {7, 31, 3, 15, 63, 1, 9, 21};
    static int p0;
    static int[] p1;
    static int[] p2;
    static int[] p3;
    static int[] p4;
    static boolean[] p5;
    static int[] p6;
    static int[] p7;
    static long[] p8;

    @Override public String fqcn() { return "PaperNativeJigsawCanAttach"; }
    @Override public String sig() { return "(I[I[I[I[I[Z[I[I[J)I"; }
    @Override public String[] methods() { return new String[]{"oldBatchSummary", "optimizedBatchSummary", "targetFirstBatchSummary"}; }

    @Override public void setup(int s) {
        switch (s) {
        case 0 -> setup(p500.Bench.N, true);
        case 1 -> setup(16, true);
        case 2 -> setup(1, true);
        case 3 -> setup(16, false);
        }
    }
    private static void setup(int n, boolean strings) {
            p0 = n;
            p1 = new int[Math.max(8, n)]; for (int i = 0; i < p1.length; i++) p1[i] = (int) ((i * 0x9E3779B1L) & 0x3FF);
            p2 = new int[Math.max(8, n)]; for (int i = 0; i < p2.length; i++) p2[i] = (int) ((i * 0x9E3779B1L) & 0x3FF);
            p3 = new int[Math.max(8, n)]; for (int i = 0; i < p3.length; i++) p3[i] = (int) ((i * 0x9E3779B1L) & 0x3FF);
            p4 = new int[Math.max(8, n)]; for (int i = 0; i < p4.length; i++) p4[i] = (int) ((i * 0x9E3779B1L) & 0x3FF);
            p5 = new boolean[Math.max(8, n)]; for (int i = 0; i < p5.length; i++) p5[i] = (i % 3) != 0;
            p6 = new int[Math.max(8, n)]; for (int i = 0; i < p6.length; i++) p6[i] = (int) ((i * 0x9E3779B1L) & 0x3FF);
            p7 = new int[Math.max(8, n)]; for (int i = 0; i < p7.length; i++) p7[i] = (int) ((i * 0x9E3779B1L) & 0x3FF);
            p8 = new long[64];
    }

    @Override public long call(int idx) {
        long acc;
        switch (idx) {
        case 0 -> { acc = PaperNativeJigsawCanAttach.oldBatchSummary(p0, p1, p2, p3, p4, p5, p6, p7, p8); }
        case 1 -> { acc = PaperNativeJigsawCanAttach.optimizedBatchSummary(p0, p1, p2, p3, p4, p5, p6, p7, p8); }
        case 2 -> { acc = PaperNativeJigsawCanAttach.targetFirstBatchSummary(p0, p1, p2, p3, p4, p5, p6, p7, p8); }
        default -> acc = 0L;
        }
        acc = acc * 31 + (dst0() ^ dst1());
        return acc;
    }
    static long dst0() {
        return p8.length > 0 ? p8[0] : 0L;
    }
    static long dst1() {
        return p8.length > 1 ? p8[1] : 0L;
    }
}
