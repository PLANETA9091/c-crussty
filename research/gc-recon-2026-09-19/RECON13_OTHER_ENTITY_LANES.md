...
## RECON-13b (TASK-325, тик 16:27): chunk-parse churn — атрибуция и ОКОННАЯ аномалия
- Полный parse-путь: BalancedPrioritisedThreadPool → ChunkLoadTask$ChunkDataLoadTask → GenericDataLoadTask$ProcessOffMainTask → SerializableChunkData.parse = **33.38% всех аллок-байтов** (10,186/30,516; DataResult-подмножество 19.4% было уже частью этого пути)
- Оконная аномалия (S7-134: cpu 0-55% / wall 55-80% / alloc 80-100%): ChunkLoadTask = **0.00% CPU / 0.00% wall / 45.03% alloc** → это НЕ steady-state churn, это BURST перезагрузки чанков в финальном окне (240-300s). Спарк-репорт чанк-статистики не содержит (activity.json = только URL профайлера)
- Вопрос RECON-13c: триггер burst (ticket churn мобов? накопленная очередь load при голодании region_threads? финальный save/reload фазы?) + rate-инструмент (диагностический лог ≠ config-win)
- Предварительный рычаг (парити-осторожно): кэш декодированных chunk-data с инвалидацией по файловой ревизии — устраняет ПОВТОРНУЮ десериализацию без изменения поведения тикетов; финальное решение после RECON-13c
