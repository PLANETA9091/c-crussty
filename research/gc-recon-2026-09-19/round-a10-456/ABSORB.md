# absorb ROUND (a10-456, run 36112171772, branch round-456-anchor-10, head 0716075)

- T1: lever=(n/a в run-env), pop=VALID, NCDFE=0, col=PARALLEL, runner=6865200 (band OK) | inside_cache=OK flush_diet=OK region_threads=OK batch_collector=OK fluid_guard=OK gc_tune=OK population_target=OK fake_players=OK fluid_bitmask=OK fluid_dirty=OK inside_bitmask=OK skip_store_bb=OK region_steal=OK bu_defer=OK -> **PASS**
- T2: threw=0, TPS-поллов=6 -> **PASS**
- T3: median=2.20 @ 6865200 (поллов=5); TPS_exp=2.24; normalized=-2.0%
- GC: young=109, Full=8, total=23.2s, avg=198ms, max=2514ms (банк-справка 18.8s/162ms/2400ms/Full=7)
- T5 ЛЕЙНЫ: total=115717 сэмплов (базлайн 115655)
  - items: 31.17% -> 29.35% (-1.82%) спад
  - fluid: 16.72% -> 16.30% (-0.41%) флэт
  - broadphase: 15.66% -> 14.75% (-0.91%) флэт
  - nav_ai: 14.16% -> 13.78% (-0.39%) флэт
  - inside_volatile: 12.01% -> 10.62% (-1.38%) спад
  - fastutil: 8.54% -> 8.66% (+0.12%) флэт
  - java_util: 7.01% -> 6.55% (-0.46%) флэт
  - paletted: 6.41% -> 6.90% (+0.49%) флэт
  - players_packets: 0.01% -> 0.00% (-0.01%) флэт

## VERDICT: **RED**
