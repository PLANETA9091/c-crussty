# absorb ROUND (round409cleg3, run 35667240319, branch round-405-c-l3, head 36ea2a3)

- T1: lever=(n/a в run-env), pop=VALID, NCDFE=0, col=PARALLEL, runner=6790872 (band OK) | inside_cache=OK flush_diet=OK region_threads=OK batch_collector=OK fluid_guard=OK gc_tune=OK population_target=OK fake_players=OK fluid_bitmask=OK fluid_dirty=OK inside_bitmask=OK skip_store_bb=OK region_steal=OK bu_defer=OK -> **PASS**
- T2: threw=0, TPS-поллов=6 -> **PASS**
- T3: median=2.20 @ 6790872 (поллов=5); TPS_exp=2.23; normalized=-1.3%
- GC: young=114, Full=9, total=22.1s, avg=179ms, max=2838ms (банк-справка 18.8s/162ms/2400ms/Full=7)
- T5 ЛЕЙНЫ: total=116165 сэмплов (базлайн 115655)
  - items: 31.17% -> 30.30% (-0.87%) флэт
  - fluid: 16.72% -> 16.45% (-0.27%) флэт
  - broadphase: 15.66% -> 15.37% (-0.28%) флэт
  - nav_ai: 14.16% -> 14.46% (+0.30%) флэт
  - inside_volatile: 12.01% -> 11.87% (-0.14%) флэт
  - fastutil: 8.54% -> 9.19% (+0.65%) флэт
  - java_util: 7.01% -> 7.03% (+0.02%) флэт
  - paletted: 6.41% -> 6.18% (-0.23%) флэт
  - players_packets: 0.01% -> 0.01% (-0.00%) флэт

## VERDICT: **RED**
