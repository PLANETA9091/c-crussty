# absorb ROUND (round-432-wgen-l5, run 35891373181, branch round-432-wgen-l5, head 898650c)

- T1: lever=(n/a в run-env), pop=VALID, NCDFE=0, col=PARALLEL, runner=6722009 (band OK) | inside_cache=OK flush_diet=OK region_threads=OK batch_collector=OK fluid_guard=OK gc_tune=OK population_target=OK fake_players=OK fluid_bitmask=OK fluid_dirty=OK inside_bitmask=OK skip_store_bb=OK region_steal=OK bu_defer=OK -> **PASS**
- T2: threw=0, TPS-поллов=6 -> **PASS**
- T3: median=2.50 @ 6722009 (поллов=6); TPS_exp=2.21; normalized=+12.9%
- GC: young=105, Full=9, total=19.7s, avg=173ms, max=2609ms (банк-справка 18.8s/162ms/2400ms/Full=7)
- T5 ЛЕЙНЫ: total=103238 сэмплов (базлайн 115655)
  - items: 31.17% -> 0.00% (-31.17%) спад
  - fluid: 16.72% -> 16.83% (+0.11%) флэт
  - broadphase: 15.66% -> 10.24% (-5.41%) спад
  - nav_ai: 14.16% -> 3.30% (-10.86%) спад
  - inside_volatile: 12.01% -> 12.67% (+0.66%) флэт
  - fastutil: 8.54% -> 6.50% (-2.04%) спад
  - java_util: 7.01% -> 7.25% (+0.23%) флэт
  - paletted: 6.41% -> 5.86% (-0.55%) флэт
  - players_packets: 0.01% -> 0.01% (+0.00%) флэт

## VERDICT: **GREEN-CANDIDATE**
