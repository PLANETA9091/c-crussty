#!/usr/bin/env python3
"""dispatch_s7148.py — dispatch leg #2''' INSIDE-CACHE (S7-148) on the
SELF-CONTAINED bridge (>= 45687c4) + deficit-driven topup fixture.

Config = preregistered leg #2' spec: census base config with inside_cache=1 —
...
Preregistered §156 gates + S7-148 NEW CHECK: 0 NoClassDefFoundError in
server-stdout; population ~148k stable whole soak (TOPUP-SCAN live).
"""
import json
import sys
import time
...
def main():
    ...
    print("dispatch POST sent (leg #2''' INSIDE-CACHE self-contained bridge); waiting...")
    for _ in range(12):
        time.sleep(5)
