...
## 8. TASK-460-02 WIRING ADDENDUM (round-460-chkswing-1, carrier cmp456_chunkmono @ d73758a3)

Свинг-вайринг (закон 13 CLIMB): scaffold h05 перенесён на носитель chunkmono,
композиция P31-агента = отдельная нога; эта нога меряет папаю ИЗОЛИРОВАННО.

- Перенос: src/papaya_shard_readers.rs (verbatim, фикс атрибута), Java-stub
  papaya/net/minecraft/server/level/PapayaShardReadOps.java — РЕФАКТОРИНГ 14f:
  nested `Pin` сплющен в raw long epoch (bridge = ZERO nested classes, colpush
  NCDFE-lesson); добавлены armNow()/isArmed() (flip строго rust-ом).
- Blob: scripts/build_papaya_ops.sh --release 8 (major 52, JDK-only classpath);
  javap flat==nested PASS (0 nested; PAPAYA_SHARD_MARK needle; major 52).
- EARLY arm-hook: src/papaya_arm.rs — EARLY-define в kernel loader (anchor
  ServerChunkCache → LivingEntity fallback) ДО первого broadphase-запроса
  (NCDFE-канон T1), selfTest → armNow → readback → маркер
  `[crussty-plugin] cmp456_chunkmono_papaya: ARMED`; fail-closed 180s.
- Engagement (risk #4 placebo-класс): после boot-quiet диагностическая
  shadow-ledger на ристре chunk-mirror событий носителя (chunk-granular JNI,
  zero per-entity JNI): COW publish + epoch-pinned lock-free read на каждый
  mirrorEvent; результаты ДИСКАРДЯТСЯ — drift-вердикт считается ТОЛЬКО по
  mutex-зеркалу (parity bit-exact vanilla при любом исходе папаи). G1-маркеры
  (pins/stableReads/publishes>0) живые; fallback-стат в steady-state ≈0.
- Честный прогноз для ЭТОЙ ноги: sidecar+shadow-ledger БЕЗ EntityLookup
  retarget → Δ ≈ 0 ± band-noise (reader-core захват +0.8..+1.4пп требует
  ретаргета broadphase-read path — отдельная миссия на chk-14-носителе;
  потолок компоненты +4.1пп не трогается этой ногой).
