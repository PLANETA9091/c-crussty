 ...
 * Artifacts (JDK-21, x3 runs): G2a frozen lockstep parity 0/300 mismatches x3 = PASS;
 * G2b unlocked race demo 0/400 anomalies x3 = INCONCLUSIVE (JMM window too narrow);
 * G2c epoch fail-closed PASS (fallbacks 316-338/400 under rebuild storm 1/8);
 * G2-perf 614400 entities: speedup 0.73-1.35x, median ~1.21x; hits identical (set parity).
 *
 * Models (javap contracts, round-396-a kernel):
 *  A) ChunkEntitySlices$EntityCollectionBySection.getEntities: plain count early-out,
 *     y-section clamp scan (minY-2.0/maxY+2.0 >>4), BasicEntityList.storage[i] scan
 *     with null/self skip, AABB.intersects, list.add.
 *  B) BasicEntityList.remove = shift-left arraycopy -> unlocked concurrent readers
 *     observe shifted/duplicated entries (JMM race, plain fields, no fences).
