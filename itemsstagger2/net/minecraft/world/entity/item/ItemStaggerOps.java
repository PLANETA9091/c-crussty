...
/**
 * ITEMS-STAGGER bridge v2 (ROUND-397 vector B re-arm, TASK-397-B).
 *
 * Same architectural replacement of the item-entity per-tick heavy-check
 * SCHEDULE as ROUND-396-B, re-armed after the census fix:
 *
 *   due(e, unit) = ((e.tickCount / unit) + e.getId()) % N == 0
 *
 * where `unit` is the caller's vanilla grid size (40 merge / 4 move / 1
 * noCollision), so the phase is always computed on exact divisions. N is
 * the phase multiplier: the round-397 dispatch convention passes
 * CRUSSTY_LEVER_ARG=1 (variant selector) -> the designed N=4 schedule;
 * an explicit ARG>1 selects that N directly.
 *
 * Census fix vs ROUND-396-B (real kernel bytes, purpur-1.21.10 javap):
 *   - mergeWithNeighbours is PRIVATE but called from tick via
 *     invokevirtual (JEP 181 nestmates), owner=ItemEntity;
 *   - `this.move(...)` is an INHERITED call: javac emits the receiver's
 *     static type as the Methodref owner -> `invokevirtual ItemEntity.move`
 *     (NOT Entity.move — the round-1 census missed it and the lever went
 *     fail-closed dormant);
 *   - Level.noCollision(Entity,AABB)Z — invokevirtual, owner=Level.
 * The rust retarget now scans invokevirtual+invokespecial and matches the
 * move site owner-tolerantly (ItemEntity OR Entity).
 *
 ...
 *   2. move(MoverType,Vec3)V       -> move(Entity,MoverType,Vec3)V
 *      only the vanilla settled-slow branch (recomputed exactly:
 *      onGround && horizontalDistanceSqr < 1e-5) gets 4 -> 4*N;
 *      moving/falling items are NEVER extra-skipped. The bridge re-dispatches
 *      through the public Entity.move — ItemEntity declares no move override,
 *      so the virtual dispatch lands exactly where vanilla's call resolved.
 ...
 *
 * Flag gate lives on the rust side (CRUSSTY_LEVER_FLAG == "items_stagger2").
 * Dormant lever = vanilla by construction (no retarget ever installed).
 */
public final class ItemStaggerOps {

    /** Phase multiplier (CRUSSTY_LEVER_ARG>1, else the designed default 4). */
    ...
    static {
        ...
        if (mh == null) {
            ...
            throw new IllegalStateException("items_stagger2: cannot build private mergeWithNeighbours handle — lever must not arm");
        }
        MERGE_VANILLA = mh;
        System.out.println("[crussty-items-stagger2] ops initialized N=" + N + " mergeHandle=ok");
    }
    ...
    public static String signature() {
        return "items_stagger2 N=" + N + " mergeHandle=ok";
    }
    ...
    public static void mergeWithNeighbours(ItemEntity e) {
        ...
        try {
            ...
            throw new RuntimeException("items_stagger2: vanilla merge dispatch failed", t);
        }
    }
