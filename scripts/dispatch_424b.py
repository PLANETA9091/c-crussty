#!/usr/bin/env python3
"""dispatch_424b.py — TASK-424-B: 3 legs l1/l2/l3 @round-424-b-inside lever=cmp424_inside."""
import json, re, subprocess, sys, time, urllib.request

REPO = "PLANETA9091/c-crussty"
API = "https://api.github.com"
WF = "world-bench-parallel.yml"
WORKTREE = "/home/z/rounds/ROUND-423/agent-b"
BRANCH = "round-424-b-inside"
...
# РОВНО банк round-424 (BOTTLENECK-424 / dispatch_anchors_424.py), lever_flag = cmp424_inside.
INPUTS = {
    ...
    "lever_flag": "cmp424_inside", "lever_arg": "1",
}
...
def dispatch(leg, ref, tok):
    api(tok, f"/repos/{REPO}/actions/workflows/{WF}/dispatches",
        method="POST", data={"ref": ref, "inputs": dict(INPUTS)})
    print(f"{leg}: dispatch sent ref={ref} lever={INPUTS['lever_flag']}")
    for _ in range(15):
        time.sleep(5)
...
def main():
    tok = token()
    sha = local_sha()
    print(f"carrier {BRANCH} @ {sha[:7]}")
    out = {}
    for leg, ref in LEGS.items():
        if not ensure_alias(tok, ref, sha):
            continue
        time.sleep(2)
        rid = dispatch(leg, ref, tok)
        if rid:
            out[leg] = rid
