#!/usr/bin/env bash
# build_455a_blobs_all.sh — TASK-455-A: rebuild ALL lever blobs touched by the
# R4 despawn+spawn+activation carrier union (cmp455_spawn added STRICT-OR into
...
# Usage: scripts/build_455a_blobs_all.sh [javac]
set -euo pipefail
cd "$(dirname "$0")/.."
...
# javap ground-truth: every rebuilt blob must show cmp455_spawn in its constant
# pool (raw-byte grep — indy recipes hide concat flags from javap -c, x93).
echo "== raw-byte cp grep cmp455_spawn =="
for b in \
...
assert b'cmp455_spawn' in b, f"STALE: {sys.argv[1]} lacks cmp455_spawn"
print(f"  OK {sys.argv[1]}: cmp455_spawn in cp")
PY
done

echo "== 455a blob rebuild OK (one javac pass, cp=full, major 65) =="
