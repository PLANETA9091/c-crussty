#!/usr/bin/env python3
"""TASK-457-G step-1: STRICT-OR cmp457_paldelta into all gate sites carrying
master cert needles cmp436_ins4/cmp451_senseins (mechanics = round-457c-eqsnap2
9d71b461 add_eqsnap2_gates_457.py; lineage 47ea8b20/fe7e7120/c25782b4; mirror-
drift урок ×451/×452: rust-гейты + java flag-lists правятся СИНХРОННО).
Existing needles preserved verbatim. NCDFE-канон ×456: entity_query::flag_enabled
(the EntityGoalQueryOps EARLY-define gate) получает cmp457_paldelta через тот же
Some-arm pass — arm AFTER define, define-race структурно исключён (097def9d).
"""
import re, sys, glob

FLAG = "cmp457_paldelta"
...
for path in java_files:
    ...
    for i, line in enumerate(lines):
        ...
        if path.endswith("ColpushOps.java"):
            # FLAG-constant style (next free slot FLAG15; chain extension at the
            # FLAG6 site — same mechanical shape as add_eqsnap2_gates_457.py)
            if 'f.trim().equals(FLAG6)' in line:
                lines[i] = line.replace('f.trim().equals(FLAG6)',
                                        f'f.trim().equals(FLAG6) || f.trim().equals(FLAG15)')
                changed = True
            elif re.match(r'\s*private static final String FLAG6 = "cmp451_senseins";', line):
                indent = re.match(r'(\s*)', line).group(1)
                lines[i] = (line.rstrip("\n") + "\n"
                            + f'{indent}/** TASK-457-G: paletted-delta demux completion carrier '
                              f'(STRICT-OR). */\n'
                            + f'{indent}private static final String FLAG15 = "{FLAG}";\n')
                changed = True
            continue
