#!/usr/bin/env bash
# build_424c_blobs_all.sh — TASK-425-C (round-424-c-chunk): rebuild ALL lever
# blobs touched by the cmp424_chunksend carrier retag (law 7 union).
# Lesson ×93 ×N: stale blob = sleeping plane; flat AND nested paths both
# installed — include_bytes! embeds the NESTED path.
#
...
# Bridges whose SOURCES this round touched (union retag only — bodies
# byte-identical to the merged mega/queryplane carriers):
#   ColpushOps / MobPushOps / MobScanOps / MobAiOps / ItemEntityManager /
#   EntityGoalQueryOps / QueryPlaneOps
# ChunkParseOps is rebuilt by scripts/build_chunkparse_ops.sh (own stubs cp).
# Usage: scripts/build_424c_blobs_all.sh [javac]
set -euo pipefail
cd "$(dirname "$0")/.."
...
echo "== 424c blob rebuild OK (7 bridges, one javac pass, cp=full, major 65) =="
