...
BATCH = [
    ("chunkmono-3", "round-456c-chunkmono-3", "round-456c-chunkmono", "cmp456_chunkmono"),
    ("chunkmono-4", "round-456c-chunkmono-4", "round-456c-chunkmono", "cmp456_chunkmono"),
]

EXPECTED_SHA = {
    "round-456c-chunkmono": "097def9d",  # NCDFE-fix: cmp456_chunkmono into flag_enabled define-gate
}
...
def main():
    ...
    print("=== TASK-456-C CHUNKMONO NCDFE-FIX RE-ROLL (legs 3/4) ===", flush=True)
    for base in sorted({b for _, _, b, _ in BATCH}):
        ...
        print(f"preflight OK: {base} @ {live} (NCDFE-fix commit)", flush=True)
    if dry:
    ...
    for leg, branch, base, lever in BATCH:
        ...
        time.sleep(4)
    print("=== TASK-456-C RE-ROLL COMPLETE: 2 диспатчей ===", flush=True)
