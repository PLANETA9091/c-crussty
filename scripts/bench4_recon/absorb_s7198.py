#!/usr/bin/env python3
"""absorb_s7197.py - TASK-371: absorb S7-172 P2-OFFLOAD v1 leg s7197
(bank v3 + region_steal=2 = MAIN-OFFLOAD static: 4 helpers tick ALL buckets,
main orchestrates; travel_diet=0) and deliver the REGRESSION-PROTOCOL
DUAL-BAR verdict vs ANCHOR-SLOW (s7184: median5 1.60 @ runner 6680195).
FORK EVIDENCE: RECON-37 threaded wall (s7196 re-roll 35488526730, digest
98ae00f6): worker duties 67.4/66.3/68.2%, I=1.01 <= 1.15 OFFLOAD-READY
(prereg RECON-36 §4: ceiling +25..33% wall-clock).
...
PREREGISTER GATES (dispatch_s7197.py):
  PG-T1 delivery: region_steal=2 (S7-172 MAIN-OFFLOAD ARMED) + travel_diet=0 +
    zero_alloc=0 + skip_store_bb=0 + bu_defer=0 + inside_cache=1 + flush_diet=1 +
    region_threads=4 + batch_collector=1, NCDFE=0, pop 150k VALID,
    MODE MARKER "workers=4" telemetry present (batch_collector prints the
    slot count every TELEMETRY_INTERVAL — the phase must have run)
  ...
  PG-T3 REGRESSION DUAL-BAR: runner within wide band 6.0M..9.5M (upstream-gated);
    ...
    (min-of-2) -> banking v4 = v3 + main_offload; else LANE-OPEN -> P2
    continuation per RECON-37: v2 residual overlap (tracker/random — parity
    review first) or REBALANCE if the 4-worker I degrades
...
CRASH-REFUTED (NPE/throw markers -> rollback region_steal default 0
(bit-identical legacy path) + verdict doc + RECON-37 fork re-check) /
INFRA-FLAKE
...
RUN_DIR = os.path.join(RESDIR, "run-s7198-mainoffload-v2-pump")
...
def main():
    ...
    if not run_id:
        raise SystemExit("usage: absorb_s7197.py <run_id>")
    ...
    rep = [f"# absorb S7-174 P2-OFFLOAD v2 PUMP s7198 (run {run_id}, head "
    ...
    if stdout_txt:
        # ---- PG-T1 delivery (S7-174 PUMP-OFFLOAD armed + bank v3 rest)
        want = {"region_steal": "3", "travel_diet": "0", "zero_alloc": "0",
        ...
        arm = "workers=4" in stdout_txt  # batch_collector telemetry: slots live
        ...
        if med is not None and runner and band_ok:
            ...
            if nd >= 0.10 and ad >= 0.10:
                ...
                rep.append("  -> **CANDIDATE GREEN** -> подтверждающий лег min-of-2 "
                           "(dispatch_s7197.py повторно) -> banking v4 = v3 + "
                           "main_offload (S7-172)")
            else:
                verdict = "LANE-OPEN"
                rep.append("  -> **< +10% хотя бы по одной оси** -> лейн ОТКРЫТ -> "
                           "P2-продолжение по RECON-37: v2 residual overlap "
                           "(tracker 2.2% — парити-ревью; random/block 1.4% — "
                           "RNG-риск) или REBALANCE, если I деградировал на "
                           "4 хелперах — реализация+диспатч в том же тике")
        elif not band_ok:
        ...
        if verdict is None:
            if band and crash == 0 and fixture == 0:
                ...
                rep.append("  -> **BAND-DISCARD** (S7-96d fast-fail экстремального "
                           "лендинга, не вердикт) — ре-диспатч dispatch_s7197.py "
                           "(макс 2 подряд)")
            elif crash:
                verdict = "CRASH-REFUTED"
                rep.append("  -> **CRASH-REFUTED** — руут-кауз по job-логу, rollback "
                           "region_steal (дефолт 0 = legacy bit-identical), "
                           "вердикт-док, RECON-37 развилка ре-чек")
            else:
                verdict = "INFRA-FLAKE"
