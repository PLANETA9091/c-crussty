#!/usr/bin/env python3
"""dispatch_round421_base.py — TASK-421: якоря ×3 fresh @master(2d23f45)
...
LEGS = {
    "anchora": ("round-421-anchora", "MASTER", ""),
    "anchorb": ("round-421-anchorb", "MASTER", ""),
    "anchorc": ("round-421-anchorc", "MASTER", ""),
}
...
def main():
    ...
    out = [f"anchors-421 @master {sha} lever=''"]
    for run in runs.get("workflow_runs", []):
        if run["name"] == "world-bench-round" and run["head_branch"].startswith("round-421-anchor") \
    ...
    with open("/home/z/rounds/ROUND-421/ANCHOR_DISPATCH.txt", "w") as f:
        f.write("\n".join(out) + "\n")
