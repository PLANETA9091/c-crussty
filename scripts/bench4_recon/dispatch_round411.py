#!/usr/bin/env python3
"""dispatch_round411.py — TASK-411: якоря ×3 @master(a3c92ab). Alias-refs round-411-*."""
...
LEGS = {
    "anchora": ("round-411-anchora", ""),
    "anchorb": ("round-411-anchorb", ""),
    "anchorc": ("round-411-anchorc", ""),
}
...
def main():
    ...
    master = subprocess.run(["git", "-C", "/home/z/c-crussty", "rev-parse", "origin/master"],
                            capture_output=True, text=True).stdout.strip()
    for name in ("round-411-anchora", "round-411-anchorb", "round-411-anchorc"):
        ensure_alias(tok, name, master)
    out = {}
    for leg, (ref, lever) in LEGS.items():
