#!/usr/bin/env python3
"""dispatch_round417_anchors.py — TASK-419: якоря ×3 fresh @master(87fc0cb).
...
LEGS = {
    "anchora": ("round-419-anchora", ""),
    "anchorb": ("round-419-anchorb", ""),
    "anchorc": ("round-419-anchorc", ""),
}
...
def wait_started(tok, min_created, tag):
    for _ in range(40):
        ...
        for run in runs.get("workflow_runs", []):
            if run["name"] == "world-bench-round" and run["head_branch"].startswith("round-419-") \
...
def main():
    ...
    for tag, (ref, lever) in LEGS.items():
        ensure_alias(tok, ref, master_sha)
    ...
    with open("/home/z/rounds/ROUND-419/ANCHORS_DISPATCH.txt", "w") as f:
