#!/usr/bin/env python3
"""dispatch_round409_c.py — TASK-409-C (vector eindex): cleg2b (re-roll after
BAND-DISCARD cleg2, now on dormant-gate fix 36ea2a3) + cleg3 (min-of-3 second
leg), both cmp405_eindex @ round-405-c-eindex HEAD. Alias-refs per-ref."""
...
LEGS = {
    "cleg2b": ("round-405-c-l2b", "cmp405_eindex", "origin/round-405-c-eindex"),
    "cleg3":  ("round-405-c-l3",  "cmp405_eindex", "origin/round-405-c-eindex"),
}
...
def ensure_alias(tok, name, src):
    ...
    if r.get("object", {}).get("sha") == sha:
        ...
        return True
    if r.get("object", {}).get("sha"):
        # ref exists at a DIFFERENT sha — must not silently repoint
        print(f"{name}: EXISTS at {r['object']['sha'][:7]} != wanted {sha[:7]} — ABORT", file=sys.stderr)
        return False
