#!/usr/bin/env python3
"""dispatch_round412b.py — TASK-412-B (v16): нога agent-b round-412-b-r1
(R4 root-cause fix: InsideRustOps arm param-count 6 + entity_compose rng
STAGE 0). Alias-refs round-412-* для per-ref параллели."""
...
LEGS = {
    "b1": ("round-412-b-r1", "round-412-b-r1", "cmp411_insidebat"),
}
...
def main():
    ...
    refs = {
        "round-412-b-r1": subprocess.run(
            ["git", "-C", "/home/z/c-crussty", "rev-parse", "round-412-b-r1"],
            capture_output=True, text=True).stdout.strip(),
    }
    # worktree sha authority: push already landed; cross-check remote ref
    remote = api(tok, f"/repos/{REPO}/git/ref/heads/round-412-b-r1")
    if remote.get("object", {}).get("sha"):
        refs["round-412-b-r1"] = remote["object"]["sha"]
