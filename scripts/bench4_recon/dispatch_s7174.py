#!/usr/bin/env python3
"""dispatch_s7174.py — dispatch of the INSIDE-DIET lever #12 v1 gate leg (TASK-332): s7172 verified but the served BlockPos bytes FAILED JVM VERIFICATION at retransform (JVMTI 62): static load chain was 1-based (receiver-convention) — fixed 4ddb10f (0-based slots, local retransform probe RETRANSFORM OK + SEMANTIC OK), same inputs, same gates
...
PREREGISTER GATES (absorb; A/B vs fresh same-bank profile s7173 = 35444005075; lane base RECON14C: inside-blocks 31.18% alloc / 9.20% CPU
and banked v3 legs):
