#!/usr/bin/env python3
"""dispatch_round415a.py — TASK-415-A: мега-композит cmp415_mcomp ×3 @round-415-a-mcomp head.
mcomp = multi(f7d04d2 cmp409_multi) ⊕ race-fix REFENCE(109e4d7) ⊕ queryplane(125c474).
refs → голова round-415-a-mcomp (НЕ master). Банк inputs exact, lever_arg=1,
travel_diet/fluid_dirty_ledger НЕ слать.
"""
...
LEGS = {
    "mc1": ("round-415-a-mc1", "cmp415_mcomp"),
    "mc2": ("round-415-a-mc2", "cmp415_mcomp"),
    "mc3": ("round-415-a-mc3", "cmp415_mcomp"),
}

HEAD_REPO = "/home/z/rounds/ROUND-415/agent-a"
...
def main():
    ...
    head = subprocess.run(["git", "-C", HEAD_REPO, "rev-parse", "HEAD"],
                          capture_output=True, text=True).stdout.strip()
    print(f"mcomp head = {head[:7]}")
