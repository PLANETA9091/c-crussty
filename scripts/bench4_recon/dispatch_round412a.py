#!/usr/bin/env python3
"""dispatch_round412a.py — TASK-412-A restart: refsync (ReferenceList mutator
fence) leg ak5c ×1 @round-412-a-r1 (HEAD 109e4d7, emap 12-site fence ⊕
navpool hardening ⊕ REFSYNC 22-site ReferenceList fence, флаг cmp405_navplane).
Inputs = РОВНО банковские (radius 640, sec 300, fp 4, pop 150000/seed 42,
band 6.0-9.5M, region_threads 4). НЕ шлём travel_diet/fluid_dirty_ledger.
Alias-refs: sha резолвится через API; ensure_alias ПАТЧИТ существующий alias
(урок dispatch_round412cv3: старый ensure_alias молча фейлил POST и диспатчил
против устаревшего sha)."""
...
LEGS = {
    "ak5c": ("round-412-a-r1", "round-412-a-r1", "cmp405_navplane"),
}
