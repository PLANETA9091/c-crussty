#!/usr/bin/env python3
"""dispatch_round414a.py — TASK-414-A: queryplane ⊕ REFENCE legs b2p1/b2p2.
Base = round-414-a-b2p head (queryplane 125c474 ⊕ race-fix 109e4d7).
Оба alias-ref → ТЕКУЩИЙ HEAD ветки round-414-a-b2p (ensure_alias).
Inputs = банк exact (radius 640 / seconds 300 / fake_players 4 / population 150000
/ seed 42 / xmx 10G / xms 4G / band 6.0-9.5M / lever_arg 1), БЕЗ travel_diet /
...
LEGS = {
    "b2p1": ("round-414-a-b2p1", "cmp414_b2p"),
    "b2p2": ("round-414-a-b2p2", "cmp414_b2p"),
}
...
def main():
    ...
    head = subprocess.run(["git", "-C", "/home/z/rounds/ROUND-414/agent-a", "rev-parse", "HEAD"],
                          capture_output=True, text=True).stdout.strip()
    print(f"round-414-a-b2p HEAD = {head[:7]}")
