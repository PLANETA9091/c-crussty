#!/usr/bin/env python3
"""dispatch_round421_a.py — TASK-421-A: ноги ×3 @round-421-a-brain(HEAD)
lever=cmp421_brain, банк inputs EXACT, band 6.0-9.5M."""
...
LEGS = {
    "l1": ("round-421-a-brain-l1", "BRANCH", "cmp421_brain"),
    "l2": ("round-421-a-brain-l2", "BRANCH", "cmp421_brain"),
    "l3": ("round-421-a-brain-l3", "BRANCH", "cmp421_brain"),
}
...
def main():
    ...
    sha = subprocess.run(["git", "-C", "/home/z/rounds/ROUND-421/agent-a", "rev-parse", "HEAD"],
                         capture_output=True, text=True).stdout.strip()
    if len(sha) != 40: raise SystemExit(f"bad branch sha {sha!r}")
    print(f"round-421-a-brain HEAD = {sha}")
    ...
    out = [f"brain-legs @round-421-a-brain {sha} lever=cmp421_brain"]
    for run in runs.get("workflow_runs", []):
        if run["name"] == "world-bench-round" and run["head_branch"].startswith("round-421-a-brain-l") \
    ...
    with open("/home/z/rounds/ROUND-421/A_DISPATCH_421A.txt", "w") as f:
        f.write("\n".join(out) + "\n")
