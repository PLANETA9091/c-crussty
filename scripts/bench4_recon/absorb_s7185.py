#!/usr/bin/env python3
"""absorb_s7185.py - TASK-341: absorb lever #14 TRAVEL-ALLOC-DIET v1 leg s7182
...
PREREGISTER GATES (dispatch_s7185.py):
...
RUN_DIR = os.path.join(RESDIR, "run-s7185-travel-diet-pair")
...
def main():
    ...
    if not run_id:
        raise SystemExit("usage: absorb_s7185.py <run_id>")
        ...
        if med is not None and runner and pair_ok:
            ...
            if nd >= 0.10 and ad >= 0.10:
                ...
                rep.append("  -> **CANDIDATE GREEN** -> подтверждающий лег s7183 "
                           "(min-of-2: dispatch_s7185.py повторно) -> banking v4")
            else:
                verdict = "LANE-OPEN"
        ...
        if verdict is None:
            if band and crash == 0 and fixture == 0:
                ...
                rep.append("  -> **BAND-DISCARD** (S7-96d fast-fail, не вердикт) — "
                           "ре-диспатч dispatch_s7185.py без анализа")
            elif crash:
