#!/usr/bin/env python3
"""dispatch_round417c.py — TASK-417-C: queryplane-awake legs ×3 on the cvs
carrier (round-417-c-bq @<head>). Lever cmp417_bq, band 6.0-9.5M, банк inputs
EXACT (as anchors template; NO travel_diet/fluid_dirty_ledger additions).
"""
...
LEGS = {
    "bqa": ("round-417-c-bqa", "cmp417_bq"),
    "bqb": ("round-417-c-bqb", "cmp417_bq"),
    "bqc": ("round-417-c-bqc", "cmp417_bq"),
}
...
def wait_started(tok, min_created, tag):
    for _ in range(40):
        time.sleep(10)
        runs = api(tok, f"/repos/{REPO}/actions/runs?event=workflow_dispatch&per_page=15")
        for run in runs.get("workflow_runs", []):
            if run["name"] == "world-bench-round" and run["head_branch"].startswith("round-417-") \
               and run["created_at"] > min_created:
                return run["id"], run["head_branch"]
    return None, None


def main():
    tok = token_from_remote()
    # TASK-417-C: alias to THIS branch head (round-417-c-bq), not master.
    head_sha = subprocess.run(["git", "-C", "/home/z/rounds/ROUND-417/agent-c", "rev-parse", "HEAD"],
                              capture_output=True, text=True).stdout.strip()
    print(f"head = {head_sha[:7]} (round-417-c-bq)")
    ...
    for tag, (ref, lever) in LEGS.items():
        ...
        time.sleep(3)
    time.sleep(20)
    runs = api(tok, f"/repos/{REPO}/actions/runs?per_page=10")
    seen = []
    for run in runs.get("workflow_runs", []):
        if run["name"] == "world-bench-round" and run["head_branch"].startswith("round-417-"):
