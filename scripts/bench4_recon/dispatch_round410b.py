#!/usr/bin/env python3
"""dispatch_round410b.py — TASK-410-B: k2 sectional fluid bridge leg (cmp410_fluidsec).
Alias-refs per-ref discipline; inputs = EXACT bank (radius 640 / seconds 300 /
fake_players 4 / fluid_guard 1 ...), lever_flag=cmp410_fluidsec, lever_arg=1."""
...
LEGS = {
    "round-410-b-l2": ("round-410-b-fluidsec", "cmp410_fluidsec"),
}
...
def token_from_remote():
    url = subprocess.run(["git", "-C", "/home/z/rounds/ROUND-410/agent-b", "remote", "get-url", "origin"],
...
def local_sha(src):
    out = subprocess.run(["git", "-C", "/home/z/rounds/ROUND-410/agent-b", "rev-parse", src],
                         capture_output=True, text=True).stdout.strip()
    return out
...
def main():
    ...
    out = {}
    for alias, (src, lever) in LEGS.items():
        if not ensure_alias(tok, alias, src):
            print(f"{alias}: alias FAIL — abort", file=sys.stderr)
            sys.exit(1)
        rid = dispatch(alias, alias, lever, tok)
        out[alias] = rid
