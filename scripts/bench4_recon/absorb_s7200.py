#!/usr/bin/env python3
"""absorb_s7199.py - TASK-375: absorb GC-TUNE leg s7199 (банк v3 + gc_tune=1:
MaxGCPauseMillis=40 + IHOP=35 + G1HeapRegionSize=8m + AlwaysPreTouch) и
вердикт v8-REGRESSION DUAL BAR vs ANCHOR-SLOW (s7184: median5 1.60 @ 6680195).
...
RUN_DIR = os.path.join(RESDIR, "run-s7200-gctune-v2")
ANCHOR_TPS, ANCHOR_RUNNER = 1.60, 6680195
BAND_MIN, BAND_MAX = 6_000_000, 9_500_000
# PG-T4 preregister v2 (vs база s7198: young 163 эвакуаций/19.0s young-SLICE, total
# 19518.8ms, avg 79.34ms по 246 событиям; v2 БЕЗ pause-target -> adaptive young -> 120..200)
TOTAL_PAUSE_GATE_MS = 19_000.0
AVG_PAUSE_GATE_MS = 115.0
YOUNG_GATE = 200
YOUNG_MIN = 120
...
def main():
    ...
    if not run_id:
        raise SystemExit("usage: absorb_s7200.py <run_id>")
    ...
    rep = [f"# absorb GC-TUNE-v2 s7200 (run {run_id}, head {str(st.get('head_sha'))[:7]}) "
    ...
    if stdout_txt:
        ...
        want = {"gc_tune": "2", "region_steal": "0", "travel_diet": "0",
        ...
        if os.path.isfile(gclog):
            region8 = bool(re.search(r"Heap Region Size:\s*(8M|8192K)", open(gclog, errors="ignore").read()))
        t1_ok = all("OK" in x for x in t1) and ncde == 0 and pop_ok and arm and region8
        rep.append("- PG-T1: " + ", ".join(t1) + f", NCDFE={ncde}, "
                   f"pop={'VALID' if pop_ok else 'BAD'}, "
                   f"mode={'WORKERS4-TELEMETRY' if arm else 'MISSING'}, "
                   f"region8M={'OK' if region8 else 'BAD'} -> **{'PASS' if t1_ok else 'FAIL'}**")
        ...
        if med is not None and runner and band_ok:
            ...
            if nd >= 0.10 and ad >= 0.10:
                ...
                rep.append("  -> **CANDIDATE GREEN** -> подтверждающий лег min-of-2 "
                           "(dispatch_s7200.py повторно) -> banking v4 = v3 + gc_tune")
            else:
                verdict = "LANE-OPEN"
                rep.append("  -> **< +10% хотя бы по одной оси** -> лейн ОТКРЫТ -> "
                           "GC-лейн: потолок-док (v1+v2 эмпирика) "
        ...
        if gs:
            ...
            t4 = (full == 0 and total <= TOTAL_PAUSE_GATE_MS
                  and avg <= AVG_PAUSE_GATE_MS and YOUNG_MIN <= young <= YOUNG_GATE)
            rep.append(f"- PG-T4: young={young}, Full={full}, total_pause={total/1000:.1f}s "
                       f"(гейт <= {TOTAL_PAUSE_GATE_MS/1000:.1f}s), avg={avg:.1f}ms "
                       f"(гейт <= {AVG_PAUSE_GATE_MS:.0f}ms), young-банд {YOUNG_MIN}..{YOUNG_GATE}, max={mx:.1f}ms "
        ...
        if verdict is None:
            if band and crash == 0 and fixture == 0:
                ...
                rep.append("  -> **BAND-DISCARD** (S7-96d fast-fail, не вердикт) — "
                           "ре-диспатч dispatch_s7200.py (макс 2 подряд)")
            elif crash:
    ...
    rep.append(f"\n## VERDICT: **{verdict}**")
    out = os.path.join(RESDIR, "ABSORB_S7200.md")
    open(out, "w").write("\n".join(rep) + "\n")
    print("\n".join(rep))
    print(f"\nwritten: {out}")
