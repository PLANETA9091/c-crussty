...
RUN_DIR = os.path.join(RESDIR, "run-s7199-gctune")
ANCHOR_TPS, ANCHOR_RUNNER = 1.60, 6680195
BAND_MIN, BAND_MAX = 6_000_000, 9_500_000
# PG-T4 preregister (vs база s7198: total 19518.8ms, avg 79.34ms, max 178.7ms)
TOTAL_PAUSE_GATE_MS = 14_000.0
AVG_PAUSE_GATE_MS = 60.0
YOUNG_GATE = 320
...
def main():
    ...
    if not run_id:
        raise SystemExit("usage: absorb_s7199.py <run_id>")
    ...
    rep = [f"# absorb GC-TUNE s7199 (run {run_id}, head {str(st.get('head_sha'))[:7]}) "
    ...
    if stdout_txt:
        ...
        want = {"gc_tune": "1", "region_steal": "0", "travel_diet": "0",
        ...
        if med is not None and runner and band_ok:
            ...
            if nd >= 0.10 and ad >= 0.10:
                ...
                rep.append("  -> **CANDIDATE GREEN** -> подтверждающий лег min-of-2 "
                           "(dispatch_s7199.py повторно) -> banking v4 = v3 + gc_tune")
            else:
                verdict = "LANE-OPEN"
                rep.append("  -> **< +10% хотя бы по одной оси** -> лейн ОТКРЫТ -> "
                           "GC-под-лейны (young-mix/region-size/pause-target варьирование) "
        ...
        if verdict is None:
            if band and crash == 0 and fixture == 0:
                ...
                rep.append("  -> **BAND-DISCARD** (S7-96d fast-fail, не вердикт) — "
                           "ре-диспатч dispatch_s7199.py (макс 2 подряд)")
            elif crash:
    ...
    rep.append(f"\n## VERDICT: **{verdict}**")
    out = os.path.join(RESDIR, "ABSORB_S7199.md")
    open(out, "w").write("\n".join(rep) + "\n")
    print("\n".join(rep))
    print(f"\nwritten: {out}")
