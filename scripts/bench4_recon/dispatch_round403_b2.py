#!/usr/bin/env python3
"""dispatch_round403_b2.py — TASK-403-B2: jnibulk-нога (cmp403_jnibulk)
против якорного банка @f19d5f5 (pair-by-runner, min-of-3).

LEGS: bleg1 = round-403-b-jnibulk + lever_flag=cmp403_jnibulk.
Алиасы round-403-bleg2/bleg3 создаются на sha ветки — ре-роллы min-of-3 без
per-ref clash гварда workflow.
Inputs = РОВНО банк-константы (radius 640 / seconds 300 / fake_players 4 /
gc_tune 3 / population 150k seed 42 / xmx 10G / band 6.0-9.5M, lever_arg=1).
"""
...
LEGS = {
    "bleg1": (BRANCH, LEVER),
    "bleg2": ("round-403-bleg2", LEVER),
    "bleg3": ("round-403-bleg3", LEVER),
}
...
def ensure_aliases(tok):
    """Алиасы re-роллов на sha ветки (min-of-3 без per-ref clash)."""
    sha = branch_sha()
    for name in ("round-403-bleg2", "round-403-bleg3"):
...
def main():
    ...
    ensure_aliases(tok)
    legs = sys.argv[1].split(",") if len(sys.argv) > 1 else ["bleg1"]
    out = {}
    for leg in legs:
        if leg not in LEGS:
