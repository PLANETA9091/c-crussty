#!/usr/bin/env python3
"""dispatch_round413b.py — TASK-413-B: queryplane ноги ×2 (bp1/bp2)
(entity-query snapshot plane cmp412_b2p1 STRICT eq, ветка round-412-b-p1
@125c474 = meganav f579c1c ⊕ inside_batch dormant ⊕ queryplane).
Ref round-413-b-p1 = alias на HEAD worktree agent-b2; per-leg aliases
round-413-bp{1,2} для per-ref параллели (sha резолвится через API —
патч-апдейт если alias уже существует с другим sha). inputs банковские РОВНО."""
...
LEGS = {
    "bp1": ("round-413-bp1", "round-413-b-p1", "cmp412_b2p1"),
    "bp2": ("round-413-bp2", "round-413-b-p1", "cmp412_b2p1"),
}
...
def main():
    ...
    # round-413-b-p1 = alias на HEAD worktree agent-b2 (ветка round-412-b-p1).
    head = subprocess.run(
        ["git", "-C", "/home/z/rounds/ROUND-412/agent-b2", "rev-parse", "HEAD"],
        capture_output=True, text=True).stdout.strip()
    if not head:
        raise SystemExit("cannot resolve agent-b2 HEAD")
    ensure_alias(tok, "round-413-b-p1", head)
