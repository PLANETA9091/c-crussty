#!/usr/bin/env python3
"""dispatch_round414c2.py — TASK-414-C2: gsel-batch ноги pfb1/pfb2 @round-414-c-pfb (70ba9ac), lever cmp414_pfb.
...
LEGS = {
    "pfb1": ("round-414-c-pfb1", "cmp414_pfb"),
    "pfb2": ("round-414-c-pfb2", "cmp414_pfb"),
}
...
def main():
    ...
    master = subprocess.run(["git", "-C", "/home/z/rounds/ROUND-414/agent-c2", "rev-parse", "HEAD"],
                            capture_output=True, text=True).stdout.strip()
    print(f"master = {master[:7]}")
    for name, _ in LEGS.values():
        ensure_alias(tok, name, master)
