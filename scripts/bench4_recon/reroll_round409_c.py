#!/usr/bin/env python3
"""reroll_round409_c.py — TASK-409-C: re-roll ONE alias-ref leg after a
BAND-DISCARD (pairing discard, not a code failure). Ref must already exist;
this script NEVER repoints it — it only re-dispatches the workflow."""
...
def dispatch_existing(leg, ref, lever, tok):
    short = ref.split("/")[-1]
...
if __name__ == "__main__":
    leg, ref, lever = sys.argv[1], sys.argv[2], sys.argv[3]
    tok = token_from_remote()
    rid = dispatch_existing(leg, ref, lever, tok)
    print(json.dumps({leg: rid}))
