#!/usr/bin/env python3
"""dispatch_round412_c.py — TASK-412-C: нога round-412-c-l2 (v2 cmp411_eqsnap
dirty-дельты @24008bd, продолжение RED cl1 35691270899). Alias-ref на СВОЁМ sha
(HEAD worktree agent-C), lever_flag=cmp411_eqsnap=1, банк exact.
travel_diet / fluid_dirty_ledger НЕ шлются (слотов в workflow нет).
"""
...
WORKTREE = "/home/z/rounds/ROUND-411/agent-c"

LEGS = {
    "c_l2": ("round-412-c-l2", "cmp411_eqsnap", "HEAD"),
}
...
# ЗАПРЕЩЕНО слать: travel_diet, fluid_dirty_ledger (нет в схеме workflow)
assert "travel_diet" not in INPUTS and "fluid_dirty_ledger" not in INPUTS
...
def api(tok, url, method="GET", data=None):
    ...
    try:
        with urllib.request.urlopen(req, timeout=60, data=payload) as r:
            body = r.read()
        return json.loads(body) if body else {"_code": r.status}
...
def local_sha(src):
    out = subprocess.run(["git", "-C", WORKTREE, "rev-parse", src],
                         capture_output=True, text=True).stdout.strip()
    return out
...
def ensure_alias(tok, name, src):
    ...
    if cur == sha:
        ...
        return True
    if cur:
        # чужой/старый sha — force-move на свой (алиас принадлежит этой линии)
        code = api(tok, f"/repos/{REPO}/git/refs/heads/{name}", method="PATCH",
                   data={"sha": sha, "force": True})
        ok = bool(code)
        print(f"{name}: PATCH {cur[:7]} -> {sha[:7]} ({'ok' if ok else 'FAIL'})")
        return ok
