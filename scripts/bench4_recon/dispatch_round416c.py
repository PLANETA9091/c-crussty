#!/usr/bin/env python3
"""dispatch_round416c.py — TASK-416-C: fluid-bulk ноги f1/f2/f3 @round-416-c-fluid2 (8599c79).
lever_flag=cmp416_fluid, band 6.0-9.5M, банк inputs exact.
Usage: python3 dispatch_round416c.py f1,f2,f3
"""
...
LEGS = {
    "f1": ("round-416-c-f1", "cmp416_fluid"),
    "f2": ("round-416-c-f2", "cmp416_fluid"),
    "f3": ("round-416-c-f3", "cmp416_fluid"),
}

ALIAS_SHA = "8599c79c231ad1ddf32befc09303f512b5bf30d4"
...
def dispatch(leg, ref, lever, tok):
    ...
    for _ in range(12):
        ...
        for r in d.get("workflow_runs", []):
            if r["head_branch"] == ref and                r["created_at"] > time.strftime("%Y-%m-%dT%H:%M", time.gmtime(time.time() - 300)):
...
def main():
    ...
    print(f"alias sha = {ALIAS_SHA[:7]} (round-416-c-fluid2 head)")
    for name, _ in LEGS.values():
        ensure_alias(tok, name, ALIAS_SHA)
