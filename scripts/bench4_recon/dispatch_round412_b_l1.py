#!/usr/bin/env python3
"""dispatch_round412_b_l1.py — TASK-412-B: одна нога round-412-b-l1 на sha
агента-B (round-411-b-inside HEAD), lever cmp411_insidebat. Inputs РОВНО
по мандату (travel_diet/fluid_dirty_ledger НЕ слать — их нет в WANT)."""
...
REF = "round-412-b-l1"
LOCAL_BRANCH = "round-411-b-inside"
...
def main():
    ...
    sha = subprocess.run(["git", "-C", "/home/z/rounds/ROUND-411/agent-b",
                          "rev-parse", LOCAL_BRANCH],
                         capture_output=True, text=True).stdout.strip()
    if not sha:
        raise SystemExit("cannot resolve local HEAD sha")
    ensure_alias(tok, REF, sha)
    rid = dispatch(REF, "cmp411_insidebat", tok)
    print(json.dumps({"ref": REF, "sha": sha, "run_id": rid}))
