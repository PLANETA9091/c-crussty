#!/usr/bin/env python3
"""dispatch_round422_b.py — TASK-422-B: ноги ×3 @round-422-b-brain2(HEAD)
lever=cmp422_brain2, банк inputs EXACT, band 6.0-9.5M."""
...
LEGS = {
    "l1": ("round-422-b-brain2-l1", "BRANCH", "cmp422_brain2"),
    "l2": ("round-422-b-brain2-l2", "BRANCH", "cmp422_brain2"),
    "l3": ("round-422-b-brain2-l3", "BRANCH", "cmp422_brain2"),
}
...
def main():
    ...
    sha = subprocess.run(["git", "-C", "/home/z/rounds/ROUND-422/agent-b", "rev-parse", "HEAD"],
                         capture_output=True, text=True).stdout.strip()
    if len(sha) != 40: raise SystemExit(f"bad branch sha {sha!r}")
    print(f"round-422-b-brain2 HEAD = {sha}")
    ...
    out = [f"brain2-legs @round-422-b-brain2 {sha} lever=cmp422_brain2"]
    for run in runs.get("workflow_runs", []):
        if run["name"] == "world-bench-round" and run["head_branch"].startswith("round-422-b-brain2-l") \
    ...
    with open("/home/z/rounds/ROUND-422/agent-b/B_DISPATCH_422B.txt", "w") as f:
        f.write("\n".join(out) + "\n")
