#!/usr/bin/env python3
"""dispatch_round414b.py — TASK-414-B: blob-sync revival legs cvs1/cvs2
@round-414-b-cvs (1aec4f8 = meganav⊕eqsnap-v3 gate-revival ⊕ race-fix 109e4d7).
lever_flag=cmp414_cvs (STRICT OR: meganav-плоскости + eqsnap-плоскость),
банк inputs exact (как dispatch_round414.py), lever_arg=1,
travel_diet/fluid_dirty_ledger НЕ шлём.
"""
...
LEGS = {
    "cvs1": ("round-414-b-cvs1", "cmp414_cvs"),
    "cvs2": ("round-414-b-cvs2", "cmp414_cvs"),
}

# TASK-414-B: обе ноги на head ветки round-414-b-cvs (git -C worktree)
def branch_head():
    out = subprocess.run(
        ["git", "-C", "/home/z/rounds/ROUND-414/agent-b", "rev-parse", "origin/round-414-b-cvs"],
        capture_output=True, text=True).stdout.strip()
    if not out:
        out = subprocess.run(
            ["git", "-C", "/home/z/rounds/ROUND-414/agent-b", "rev-parse", "HEAD"],
            capture_output=True, text=True).stdout.strip()
    return out
...
def main():
    ...
    head = branch_head()
    print(f"round-414-b-cvs head = {head[:7]}")
