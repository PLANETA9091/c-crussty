...
# GoalBatchOps bridge (cmp416_gsel3 gsel-batch plane).
#
...
install_blob() { # outdir fqcn... — installs BOTH the short legacy path and
  # the full fqcn path (canonical: include_bytes! + check_blobs_sync.sh read
  # the full path; ×93/×416 lesson: stale full-path blob = javap-gate FAIL).
  ...
  for cls in "$@"; do
    ...
    dir="$outdir/$cls_dir"
    case "$outdir" in
      */"$cls_dir") : ;; # outdir уже является fqcn-каталогом (gsel) — не дублируем
      *) mkdir -p "$dir"
         cp "$ALL_BUILD/$cls.class" "$dir/$base.class"
         echo "blob: $dir/$base.class ($(stat -c%s "$dir/$base.class") bytes)" ;;
    esac
  done
}
