 ...
 * Paper «optimize entity pushes» (paper-server
 * net/minecraft/world/entity/LivingEntity.java.patch: «don't run getEntities if
 * we're not going to use its result» + «Cap entity collisions»): ванильное тело
 * LivingEntity.pushEntities потребляет результат
 ...
 *
 *   - позиции мобов пушатся в rust per-tick (mhUpsert: id + lid + центр AABB;
 ...
 *   - push-кандидаты отдаются java ОДНИМ запросом mhQuery(box, lid, out):
 *     64-шардный per-shard-seqlock грид 1.0-ячеек (шард = mix64(cell_key)&63;
 *     читатель — seqlock v1->данные->v2 Acquire с cell-local retry; писатель —
 *     per-shard spinlock с нечёт/чёт бампом версии; фикс. 16384 слота/шард,
 *     fail-closed на переполнении — src/mobhash.rs, RESEARCH-A §2);
 ...
 * (javap offset 850), поэтому само-апсерт в момент запроса кладёт в грид
 ...
 * move() пушуемого (Paper Entity push: additive deltas) ⇒ living<->living
 * свежесть точная в пределах однопроходного тика.
 *
 * FAIL-CLOSED: ENABLED (env == "cmp401_mobhash") && nativeOk (mhProbe magic)
 ...
 */
public final class MobHashOps {

    private static boolean leverEnabled() {
        String f = System.getenv("CRUSSTY_LEVER_FLAG");
        return f != null && f.trim().equals("cmp401_mobhash");
    }

    private static final boolean ENABLED = leverEnabled();

    private static final int PROBE_MAGIC = 0x4D48; // "MH"
    ...
    // ---- natives (impl: src/mobhash.rs, RegisterNatives после define) ----
    private static native int mhProbe();
    private static native int mhUpsert(int id, int lid, double x, double y, double z);
    private static native int mhRemove(int id);
    private static native int mhQuery(double qx0, double qy0, double qz0,
    ...
    private static final ThreadLocal<ArrayList<Entity>[]> RING =
            ThreadLocal.withInitial(MobHashOps::newRing);
    ...
    private static boolean probeOnce() {
        ...
        synchronized (MobHashOps.class) {
            if (nativeOk) {
                return true;
            }
            try {
                nativeOk = mhProbe() == PROBE_MAGIC;
            } catch (Throwable t) {
                nativeOk = false;
            }
            return nativeOk;
    ...
    public static List<Entity> pushables(Level level, Entity entity, AABB box) {
        ...
        int[] out = SCRATCH.get();
        int n = mhQuery(box.minX, box.minY, box.minZ, box.maxX, box.maxY, box.maxZ, lid, out);
        if (n < 0) {
            if (n <= OVERFLOW_MAX) {
                out = new int[(-n) * 4];
                SCRATCH.set(out);
                n = mhQuery(box.minX, box.minY, box.minZ, box.maxX, box.maxY, box.maxZ, lid, out);
            }
            if (n < 0) {
                if (n == ERR_STRUCT) {
    ...
    private static boolean upsertSelf(Entity e) {
        ...
        if (Math.max(hw, hh) > 1.0D) {
            ...
            LOG.warning("[crussty-plugin] cmp401_mobhash: oversized bounding radius "
        ...
        int[] box = idMap.get(e);
        if (box != null) {
            int rc = mhUpsert(box[0], lid, cx, cy, cz);
            if (rc == ERR_STRUCT) {
                broken = true;
                return true;
            }
        ...
        synchronized (ID_LOCK) {
            ...
            if (box == null) {
                ...
                int rc = mhUpsert(id, lid, cx, cy, cz);
                if (rc == ERR_STRUCT) {
    ...
    // ------------------------------------------------------------------
    // Graveyard sweep: isRemoved()-энтити -> mhRemove + freeId (каденция
    ...
    private static void maybeSweep() {
        ...
        try {
            ...
            LOG.warning("[crussty-plugin] cmp401_mobhash: sweep failed: " + t);
        } finally {
            SWEEPING.set(false);
        }
    }
