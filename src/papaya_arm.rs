...
//! (TASK-460-02, round-460-chkswing-1; swing layer on the cmp456_chunkmono
//! carrier d73758a3; sources transferred from round-459-h05 scaffold).
//!
...
//! cmp456_chunkmono_papaya: ARMED` LAST. Any failure => fail-closed dormant.
//!
//! Engagement (lesson-408 / risk #4 "спящий гейт = placebo"): after boot
//! quiet + arm, the rust L2 chunk-mirror event stream (chunk-granular JNI,
//! law 6 — ZERO per-entity JNI) drives the papaya lock-free machinery for
//! real: every mirrorEvent does a COW publish + epoch-pinned lock-free read
...
const LEVER_ID: &str = "cmp456_chunkmono_papaya";
...
/// STRICT-OR swing-lever gate (TASK-461-65, swarx-3): the papaya layer arms
/// on its own lever id OR on the swarx composite carrier id cmp458_swar —
/// the certified +18.0 swarx-1 carrier this leg composes onto (mirror-drift
/// lesson x452: prod gates and swing levers move synchronously; the
/// union-widen canon d22835bd rides the SAME carrier id on every plane's
/// STRICT-OR list, so cmp458_swar = the whole swarx stack + this sidecar).
fn enabled() -> bool {
    matches!(
        std::env::var("CRUSSTY_LEVER_FLAG").as_deref(),
        Ok(LEVER_ID) | Ok("cmp458_swar")
    )
}

/// True once the sidecar is defined+armed AND boot quiet passed — the gate
/// for the diagnostic shadow-ledger engagement in chunk_sched::mirror_event.
...
/// Results are DISCARDED (diagnostic ledger): the mutex drift path in
/// chunk_sched::mirror_event is untouched, so parity is bit-exact vanilla
