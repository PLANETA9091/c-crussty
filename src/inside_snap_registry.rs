...
//! climb-флаг `cmp456_chunkmono_p31snap` (TASK-460-01) ИЛИ носитель-паспорт
//! `cmp457_paldelta` (TASK-461-71, paldelta-14: sidecar едет на paldelta-нос,
//! канон inside_snap.rs/inside_bitmask.rs — их STRICT-OR списки уже содержат
//! cmp457_paldelta; пустой/чужой = ваниль бит-в-байт: класс не определяется,
//! хуков нет). Legacy env CRUSSTY_SNAPREG=1 принимается для A/B-реплеев
//! (канон inside_bitmask).
//! Прогноз (карточка ID-P32): java_util −40-60% CHM-части => +0.8-1.2пп —
//! но ТОЛЬКО при v2-вайринге serve-пути; v1 (этот код) = armed scaffold БЕЗ
//! ретаргетов (parity-лега по конструкции, TASK-461-71).
//!
//! DEFINE-RACE СЕРИАЛИЗАЦИЯ (TASK-461-71, критично для paldelta-носителя):
//! на смешанном флаге CHM-плоскость (inside_snap.rs) армится ТОМ ЖЕ левером
//! и сама определяет $Snap/$Lane. Слепой параллельный define = гонка, где
//! проигравший ПОЙЗОНИТ другого (наш define раньше => CHM define_class($Snap)
//! ловит LinkError и ВЕСЬ CHM-лейн абортится fail-closed; их define раньше =>
//! мы абортимся). Решение: после boot даём CHM-плоскости фору через её pub
//! wait_bridge_ready (BRIDGE_READY у inside_snap stored ПОСЛЕ define_bridge +
//! selfTest, ДО secWrite-патча => точный сигнал «$Snap/$Lane определены»);
//! при ready — РЕЮЗАем (define скипается); при таймауте (чистый cmp459_snapreg
//! A/B, CHM не армится вовсе) — определяем сами; LinkError на define трактуем
//! как CHM-reuse (единственная причина — их define тех же байтов:
//! InsideSnapOps.java неизменен носитель↔источник; совместимость полей
//! валидирует selfTest ДО arm — fail-closed сохранён).
    ...
    let lever = std::env::var("CRUSSTY_LEVER_FLAG")
        .map(|v| {
            ...
            v == "cmp459_snapreg"
                ...
                // TASK-461-71 (paldelta-14): ride the paldelta carrier id —
                // same STRICT eq string the whole cmp457_paldelta stack gates
                // on (inside_snap.rs/inside_bitmask.rs/tickplane/... canon).
                || v == "cmp457_paldelta"
        })
        .unwrap_or(false);
    if lever {
...
pub fn activate() {
    ...
    std::thread::spawn(|| {
        ...
        // TASK-461-71 define-race serialization (header): on a mixed lever
        // (CHM plane armed by the SAME carrier string) let inside_snap.rs
        // finish its define_bridge+selfTest first, then reuse its nested
        // types. BRIDGE_READY inside_snap stores AFTER define_bridge => the
        // exact "nested types are defined" signal. Timeout = CHM plane not
        // arming (pure cmp459_snapreg A/B or CHM failed) => we self-define.
        let chm_expected = crate::inside_snap::enabled_pub();
        let chm_ready = chm_expected && crate::inside_snap::wait_bridge_ready(90_000);
        ...
        let defined = cplug_sdk::jni_util::with_attached(|env| {
            ...
            //
            // TASK-461-71 define-race serialization: if the CHM plane armed
            // on this lever and published BRIDGE_READY (its define_bridge+
            // selfTest completed => $Snap/$Lane ARE defined), REUSE them —
            // a parallel define here is the race whose loser poisons the
            // other bridge (see header). Otherwise attempt the define; a
            // duplicate (LinkError) is treated as CHM-owned reuse (only
            // possible cause: inside_snap.rs defined the IDENTICAL bytes —
            // InsideSnapOps.java unchanged carrier↔source); any real break
            // is caught by selfTest BEFORE arm (fail-closed preserved).
            let (snap_ok, lane_ok) = if chm_ready {
                eprintln!(
                    "[crussty-plugin] inside_snapreg: CHM plane owns {SNAP_NESTED_CLASS}/{LANE_NESTED_CLASS} on this lever — reusing (race-free define-order, TASK-461-71)"
                );
                (true, true)
            } else {
                let s = define_nested(env, SNAP_BYTES, SNAP_NESTED_CLASS, gref)
                    || {
                        eprintln!(
                            "[crussty-plugin] inside_snapreg: define_class({SNAP_NESTED_CLASS}) duplicate -> treating as CHM-owned reuse (selfTest validates)"
                        );
                        true
                    };
                let l = define_nested(env, LANE_BYTES, LANE_NESTED_CLASS, gref)
                    || {
                        eprintln!(
                            "[crussty-plugin] inside_snapreg: define_class({LANE_NESTED_CLASS}) duplicate -> treating as CHM-owned reuse (selfTest validates)"
                        );
                        true
                    };
                (s, l)
        ...
        eprintln!(
            "[crussty-plugin] inside_snapreg: TASK-461-71 paldelta-14 marker (lever={}; nested-owner={}; v1: no retarget sites => carrier-parity by construction)",
            std::env::var("CRUSSTY_LEVER_FLAG").unwrap_or_default().trim(),
            if chm_ready { "CHM-reuse" } else { "sidecar-define" }
        );
