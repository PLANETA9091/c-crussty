...
// ---------------------------------------------------------------------------
// Gate (canon: inside_cache.rs register/activate discipline).
//
// TASK-460-40 WIRED: the classifier is carried by the INSIDE-BATCH bridge
// (InsideBatchOps.quantumServe — K-tick rest skip + fresh swept-segment
// serve; dirty-mutants mandatory via live-state traversal). This module
// stays the pure reference model + arm-gate: with the lever off NOTHING
// arms (byte-indistinguishable from pre-P34).
// ---------------------------------------------------------------------------

fn enabled() -> bool {
    // TASK-460-40: climb composite on the chunkmono carrier (STRICT eq).
    if std::env::var("CRUSSTY_LEVER_FLAG")
        .map(|v| v.trim() == "cmp456_chunkmono_p31quant")
        .unwrap_or(false)
    {
        return true;
    }
...
/// Register (idempotent, call once from cplugin_init). No byte hook of its
/// own: the classifier rides the inside_batch bridge (TASK-460-40 wiring);
/// the arm happens inside inside_batch::activate (define → natives →
/// noteBatchArmed → quantumSelfTest → noteQuantumArmed → BRIDGE_READY).
pub fn register() {
    if !enabled() {
        eprintln!(
            "[crussty-plugin] inside_quantum_gate: dormant (set CRUSSTY_INSIDE_QUANTUM=1 or lever cmp456_chunkmono_p31quant)"
        );
        return;
    }
    eprintln!(
        "[crussty-plugin] inside_quantum_gate: quantum-rest classifier armed via INSIDE-BATCH bridge (TASK-460-40: K-tick rest skip, fresh-state sweep serve, dirty-mutants mandatory)"
    );
}

/// Background activation: the model self-test below pins the classifier
/// contract (in-process arm/skip happens inside inside_batch::activate —
/// bridge-side quantumSelfTest + noteQuantumArmed, arm-order canon).
pub fn activate() {
    if !enabled() {
        return;
    }
