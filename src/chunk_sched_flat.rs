...
//! include_bytes!("../chunksched/build/net/minecraft/server/level/ChunkSchedFlatOps.class")
...
const OPS_CLASS: &str = "net/minecraft/server/level/ChunkSchedFlatOps";
...
pub fn register() {
    ...
    if OPS_BYTES.is_empty() {
        ...
        eprintln!(
            "[crussty-plugin] {LEVER_ID}: scaffold blob empty (ChunkSchedFlatOps.class not wired) — hook stays dormant"
        );
        return;
    }
