//! Sharded seqlock spatial grid for the MOB push-broadphase (TASK-400-J —
//! vector mobpush; lever cmp399_mobpush). A SEPARATE pattern-B instance
//! (architecture ported verbatim from src/items_index.rs cmp399_shard arm):
//! 64 shards keyed by mix64(cell_key)&63, lock-free per-cell read path
//! (v1→data→v2 Acquire pair, cell-local retry with n0-rollback, retry
//! exhaustion → ERR_RANGE per-call fallback), writers under one global mutex
//! with odd/even shard-version bumps (linearized writes). Fixed memory:
//! 16384 slots/shard + 1<<20 per-id entries, never reallocated ⇒ UAF
//! impossible; overflow fails closed (ERR_STRUCT → bridge disarms → vanilla).
//!
//! UNIVERSE: living entities that run LivingEntity.pushEntities (the mob push
//! lane). Java bridge (MobPushOps) pushes per-tick SELF position (AABB center)
//! + derives the ≤1.0 bounding-radius gate Java-side; rust stores ONLY
//! id→cell membership (NEXT/CELL per-id arrays) — no objects, no AABBs.
//! The query window is the vanilla query box padded by ±1.0 per axis on the
//! CENTER coordinates: a candidate with half-extent ≤ 1.0 whose AABB
//! intersects the box has its center within ±1.0 of the box, hence its
//! floor(center) lies in floor(min)-1..floor(max)+1 (floor monotonicity,
//! integer shift: floor(a±1) = floor(a)∓1). Java re-validates every candidate
//! exactly (level, AABB.intersects, EntitySelector.pushableBy) — candidate
//! SET parity by construction; iteration order = cell-chain order
//! (documented delta, items_subsys2 class).
//!
//! FAIL-CLOSED: every native returns <0 on any inconsistency (range
//! weirdness, capacity overflow, dangling chain, missing id). The Java
//! bridge falls back to the vanilla EntityLookup fill for that call and
//! disarms the lever on structural errors. An unarmed lever never touches
//! these tables.
//!
//! THREADING: upserts come from region workers (up to W concurrent, one per
//! ticking entity — an entity ticks on exactly one worker), queries run on
//! the same workers during aiStep; all writes serialize on the global WLOCK
//! (writes ≈ 1 JNI per living entity per tick, same order the legacy
//! vanilla section scan cost a full section walk).
...
/// ≤ 1.0). Writes ids into the pinned `out` array; returns the count,
/// -(out_cap) on overflow (caller grows + retries), ERR_RANGE for absurd
/// scan widths, ERR_STRUCT on JNI trouble.
///
/// # Safety
/// See mob_probe.
#[no_mangle]
