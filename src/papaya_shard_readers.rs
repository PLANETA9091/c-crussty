...
pub fn read_shard<'a>(
    ...
    // TASK-462-62: debug_assert УДАЛЁН — он противоречил собственному
    // fail-closed контракту модуля (out-of-range => fallback None) и собственному
    // delivery-тесту out_of_range_shard_falls_back (cargo test debug-профиля
    // ловил панику). Диапазон проверяется явным if ниже — контракт тот же.
