...
fn enabled() -> bool {
    ...
    std::env::var("CRUSSTY_LEVER_FLAG").map(|v| v.trim() == "cmp457_paldelta" || v.trim() == "cmp458_swar" || v.trim() == "cmp458_swar_papaya").unwrap_or(false)
}
...
fn lever_tag() -> &'static str {
    if std::env::var("CRUSSTY_LEVER_FLAG").map(|v| v.trim() == "cmp457_paldelta" || v.trim() == "cmp458_swar" || v.trim() == "cmp458_swar_papaya").unwrap_or(false) {
        "cmp457_paldelta"
    } else {
        "paletted"
    }
