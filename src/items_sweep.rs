//! Runtime wiring for the ITEMS-SWEEP lever (TASK-396-C, MEGA-ROUND-1,
//! vector C: sweep-line batch-merge — ONE grid-sweep pass per region bucket
//! per tick replaces the per-entity mergeWithNeighbours broadphase queries;
...
//!
//! Gate (ItemEntity patch only): env `CRUSSTY_LEVER_FLAG == "items_sweep"`.
...
pub fn register() {
    if !enabled() {
        eprintln!(
            "[crussty-plugin] items_sweep: dormant (CRUSSTY_LEVER_FLAG != items_sweep)"
        );
        return;
    }
    let t = target();
    cplug_sdk::hooks::register_bytes(ITEM_ENTITY, move |_name, bytes| {
        ...
        if !READY.load(Ordering::Relaxed) {
            eprintln!(
                "[crussty-plugin] items_sweep: pristine sighting {ITEM_ENTITY} {} bytes (major {})",
                bytes.len(),
        ...
        if !t.served.swap(true, Ordering::Relaxed) {
            eprintln!(
                "[crussty-plugin] items_sweep: hook serve {ITEM_ENTITY} {} bytes",
        ...
        .spawn(move || {
            ...
            loop {
                ...
                if std::time::Instant::now() > deadline - std::time::Duration::from_secs(170) {
                    eprintln!("[crussty-plugin] items_sweep: forcing kernel load of {ITEM_ENTITY}");
                    crate::improved_noise::force_load_kernel_class(ITEM_ENTITY);
                }
                if std::time::Instant::now() > deadline {
                    eprintln!(
                        "[crussty-plugin] items_sweep: {ITEM_ENTITY} not loaded within 180s, hook stays dormant"
                    );
                    return;
                }
            ...
            if !crate::improved_noise::wait_for_boot() {
                eprintln!("[crussty-plugin] items_sweep: boot marker not seen, hook stays dormant");
                return;
            }
            ...
            if !crate::region_threads::wait_bridge_ready_pub(180_000) {
                eprintln!(
                    "[crussty-plugin] items_sweep: region_threads bridge never became ready (ItemsSweepOps co-define), hook stays dormant"
                );
                return;
            }
            ...
            if !t.orig_is_some() {
                eprintln!(
                    "[crussty-plugin] items_sweep: {ITEM_ENTITY} predates hook, capturing via no-op retransform"
                );
                for attempt in 1..=3 {
                    let _ = attempt;
                ...
                if !t.orig_is_some() {
                    eprintln!(
                        "[crussty-plugin] items_sweep: no pristine bytes for {ITEM_ENTITY}, hook stays dormant"
                    );
                    return;
                }
            }
            ...
            let (patched, outcome) = match patch_items_sweep(&original) {
                ...
                Err(e) => {
                    eprintln!(
                        "[crussty-plugin] items_sweep: patch rejected ({e}), hook stays dormant"
                    );
                    return;
                }
            };
            eprintln!(
                "[crussty-plugin] items_sweep: computed patch for {ITEM_ENTITY} ({} -> {} bytes, {outcome})",
                original.len(),
                patched.len()
            );
            ...
            crate::kernel_policy::audit_wire(SWEEP_OPS_CLASS, "tickMerge", "items_sweep v1");
            ...
            eprintln!(
                "[crussty-plugin] items_sweep: {ITEM_ENTITY} armed, retransform rc={rc}"
            );
        })
        .ok();
}
