...
pub fn register() {
    if !lever_flag_matches() {
        eprintln!(
            "[crussty-plugin] cmp424_inside: dormant (lever_flag != cmp424_inside, vanilla inside lane)"
        );
        return;
    }
    cplug_sdk::hooks::register_bytes(TARGET_CLASS, move |_name, bytes| {
        ...
        if !READY.load(Ordering::Acquire) {
            ...
            if orig.is_none() {
                eprintln!(
                    "[crussty-plugin] cmp424_inside: pristine sighting {TARGET_CLASS} {} bytes (major {})",
                    bytes.len(),
    ...
    std::thread::spawn(|| {
        ...
        loop {
            ...
            if std::time::Instant::now() > deadline {
                eprintln!(
                    "[crussty-plugin] cmp424_inside: {TARGET_CLASS} not loaded within 180s, hook stays dormant"
                );
                return;
            }
            if std::time::Instant::now() > deadline - std::time::Duration::from_secs(170) {
                eprintln!(
                    "[crussty-plugin] cmp424_inside: forcing kernel load of {TARGET_CLASS}"
                );
        ...
        if !crate::improved_noise::wait_for_boot() {
            eprintln!("[crussty-plugin] cmp424_inside: boot marker not seen, hook stays dormant");
            return;
        }
        ...
        for (name, bytes) in [(OPS_CLASS, OPS_BYTES), (SNAP_CLASS, SNAP_BYTES)] {
            ...
            if major > jvm_major {
                eprintln!(
                    "[crussty-plugin] cmp424_inside: {name} is class major {major} but JVM supports up to {jvm_major} — rebuild entityinside/ blobs; hook stays dormant"
                );
                return;
            }
        }
        ...
        if let Err(e) = classfile::inside_snap_resolution_closure(OPS_BYTES) {
            eprintln!(
                "[crussty-plugin] cmp424_inside: RESOLUTION CLOSURE FAILED: {e} — hook stays dormant"
            );
            return;
        }
        ...
        let Some(gops) = define_bridge() else {
            eprintln!(
                "[crussty-plugin] cmp424_inside: bridge definition failed, hook stays dormant"
            );
            return;
        };
        BRIDGE_READY.store(true, Ordering::Release);
        eprintln!(
            "[crussty-plugin] cmp424_inside: defined {OPS_CLASS} in kernel loader (+ Snap), natives registered"
        );
        ...
        if !selftest {
            eprintln!(
                "[crussty-plugin] cmp424_inside: selfTest FAILED — hook stays dormant (fail-closed)"
            );
            return;
        }
        ...
        let Some(orig) = target()
            ...
            eprintln!(
                "[crussty-plugin] cmp424_inside: no pristine bytes for {TARGET_CLASS} — DISARM (fail-closed: snapshots without invalidation go permanently stale)"
            );
            ...
            Ok((patched, outcome)) => match outcome {
                classfile::RetargetOutcome::Retargeted { sites } if sites == 1 => {
                    *target()
                    ...
                    eprintln!(
                        "[crussty-plugin] cmp424_inside: PATCHED {TARGET_CLASS}.setBlockState -> InsideSnapOps.secWrite (Retargeted {{ sites: {sites} }}; {} -> {} bytes; retransform rc={rc})",
                        orig.len(),
                        patched.len()
                    );
                }
...
fn define_bridge() -> Option<*mut c_void> {
    cplug_sdk::jni_util::with_attached(|env| {
        ...
        let Some(c) = env.define_class(OPS_CLASS, gref, OPS_BYTES) else {
            crate::describe_exception(env);
            eprintln!("[crussty-plugin] cmp424_inside: define_class({OPS_CLASS}) failed");
            return None;
        };
        let names = [
        ...
        if let Err(code) = reg {
            env.exception_clear();
            eprintln!(
                "[crussty-plugin] cmp424_inside: register_natives failed (code {code}) — hook stays dormant"
            );
        ...
        if let Some(s) = env.define_class(SNAP_CLASS, gref, SNAP_BYTES) {
            ...
            crate::describe_exception(env);
            eprintln!("[crussty-plugin] cmp424_inside: define_class({SNAP_CLASS}) failed");
            env.delete_local_ref(c);
            env.delete_local_ref(loader);
            env.delete_local_ref(class_cls);
            return None;
...
fn selftest(env: &jvmti_bindings::env::JniEnv, gops: *mut c_void) -> bool {
    ...
    let Some(mid) = env.get_static_method_id(cls, "selfTest", "()Z") else {
        crate::clear_exception(env);
        eprintln!("[crussty-plugin] cmp424_inside: selfTest resolution failed");
        return false;
    };
    let rc = env.call_static_int_method(cls, mid, &[]);
    let had_exc = crate::clear_exception(env);
    if had_exc {
        eprintln!("[crussty-plugin] cmp424_inside: selfTest threw — fail-closed");
        return false;
    }
    rc != 0
}
