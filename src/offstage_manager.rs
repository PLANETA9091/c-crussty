//! Runtime wiring for the OFF-THREAD STAGE lever (TASK-401-D, vector
//! offthread — lever cmp401_offthread; bridge
//! mobstage/net/minecraft/world/entity/MobStageOps.java, grid + background
//! prep stage src/mobs_offgrid.rs, retarget in src/classfile.rs
//! patch_push_entities_stage).
//!
...
//! LivingEntity once.
//!
//! Fail-closed: flag != "cmp401_offthread" → no hook installed at all (byte-
...
const OPS_CLASS: &str = "net/minecraft/world/entity/MobStageOps";

const OPS_BYTES: &[u8] =
    include_bytes!("../mobstage/build/net/minecraft/world/entity/MobStageOps.class");

/// Java-side gate baked into OPS_BYTES (<clinit>: ENABLED =
/// "cmp401_offthread".equals(trim(getenv))). The exact-match lever keeps the
...
pub fn register() {
    if lever_flag() != GATE_LEVER {
        eprintln!(
            "[crussty-plugin] mobs_offgrid: dormant (set CRUSSTY_LEVER_FLAG={GATE_LEVER} to enable)"
        );
        return;
    }
    let t = target();
    cplug_sdk::hooks::register_bytes(t.name, move |_name, bytes| {
        ...
        if !READY.load(Ordering::Acquire) {
            ...
            eprintln!(
                "[crussty-plugin] mobs_offgrid: pristine sighting {} {} bytes (major {})",
                t.name,
                bytes.len(),
        ...
        if !t.served.swap(true, Ordering::Relaxed) {
            eprintln!(
                "[crussty-plugin] mobs_offgrid: hook serve {} {} bytes",
                t.name,
    ...
    std::thread::spawn(|| {
        ...
        while cplug_sdk::classes::find_class(t.name).is_none() {
            if std::time::Instant::now() > deadline {
                eprintln!(
                    "[crussty-plugin] mobs_offgrid: {} not loaded within 180s, hook stays dormant",
                    t.name
                );
                return;
            }
        ...
        if !crate::improved_noise::wait_for_boot() {
            eprintln!("[crussty-plugin] mobs_offgrid: boot marker not seen, hook stays dormant");
            return;
        }
        ...
        if ops_major > jvm_major {
            eprintln!(
                "[crussty-plugin] mobs_offgrid: {OPS_CLASS} is class major {ops_major} but JVM supports up to {jvm_major} — rebuild mobstage/ via javac (see mobstage/); hook stays dormant"
            );
            return;
        }
        ...
        if !t.orig_is_some() {
            eprintln!(
                "[crussty-plugin] mobs_offgrid: {} predates hook, capturing via no-op retransform",
                t.name
            );
            for _ in 1..=3 {
            ...
            if !t.orig_is_some() {
                eprintln!(
                    "[crussty-plugin] mobs_offgrid: no pristine bytes for {}, hook stays dormant",
                    t.name
                );
                return;
            }
        ...
        let defined = cplug_sdk::jni_util::with_attached(|env| {
            ...
            let Some(c) = env.define_class(OPS_CLASS, gref, OPS_BYTES) else {
                crate::describe_exception(env);
                eprintln!("[crussty-plugin] mobs_offgrid: define_class({OPS_CLASS}) failed");
                return false;
            };

            // RegisterNatives: mobProbe/mobUpsert/mobRemove/stageQuery
            // (impl — src/mobs_offgrid.rs; stageQuery = off-thread stage serve). Провал регистрации → armed()=false
            ...
            let names = [
                ...
                CString::new("stageQuery").expect("no NUL"),
            ];
            let sigs = [
                CString::new("()I").expect("no NUL"),
                CString::new("(IIDDD)I").expect("no NUL"),
                CString::new("(I)I").expect("no NUL"),
                CString::new("(IIDDDDDD[I)I").expect("no NUL"),
            ];
            let natives = [
                jvmti_bindings::jni::JNINativeMethod {
                    name: names[0].as_ptr(),
                    signature: sigs[0].as_ptr(),
                    fnPtr: crate::mobs_offgrid::mob_probe as *const c_void as *mut c_void,
                },
                jvmti_bindings::jni::JNINativeMethod {
                    name: names[1].as_ptr(),
                    signature: sigs[1].as_ptr(),
                    fnPtr: crate::mobs_offgrid::mob_upsert as *const c_void as *mut c_void,
                },
                jvmti_bindings::jni::JNINativeMethod {
                    name: names[2].as_ptr(),
                    signature: sigs[2].as_ptr(),
                    fnPtr: crate::mobs_offgrid::mob_remove as *const c_void as *mut c_void,
                },
                jvmti_bindings::jni::JNINativeMethod {
                    name: names[3].as_ptr(),
                    signature: sigs[3].as_ptr(),
                    fnPtr: crate::mobs_offgrid::stage_query as *const c_void as *mut c_void,
                },
            ];
            ...
            if let Err(code) = reg {
                env.exception_clear();
                eprintln!(
                    "[crussty-plugin] mobs_offgrid: register_natives failed (code {code}) — hook stays dormant"
                );
        ...
        let Some(original) = t.take_orig() else {
            eprintln!("[crussty-plugin] mobs_offgrid: pristine bytes vanished, hook stays dormant");
            return;
        };
        let major = crate::improved_noise::class_version(&original)
            .map(|(m, _)| m)
            .unwrap_or(0);
        let (patched, outcome) = match crate::classfile::patch_push_entities_stage(&original) {
            Ok(pair) => pair,
            Err(e) => {
                eprintln!(
                    "[crussty-plugin] mobs_offgrid: pushEntities retarget rejected ({e}), hook stays dormant"
                );
                return;
            }
        };
        ...
        eprintln!(
            "[crussty-plugin] mobs_offgrid: computed patch for {} ({} -> {} bytes, {outcome:?})",
            t.name,
            original.len(),
            patched.len()
        );
        ...
        eprintln!(
            "[crussty-plugin] cmp401_offthread: ARMED offstage=bg-prep-thread grid=sharded-seqlock shards=64 window=5^3 cell=1.0 pad=2.0 radius_gate=1.0 prepcap=262144 ccap=64 co-version=cell-key+lid+window-containment miss=vanillaFill per-call (rust mobs_offgrid off-thread stage; candidate enumeration OFF the tick path; pushEntities tail untouched vanilla; ERR_RANGE fallback, disarm ERR_STRUCT)"
        );

        // Start the background prep stage only now (armed lever): an unarmed
        // lever never runs the thread.
        crate::mobs_offgrid::spawn_prep();

        // Single retransform; the callback serves the cached patch.
        crate::kernel_policy::audit_wire(OPS_CLASS, "pushables", "mobs_offgrid offstage v1");
        READY.store(true, Ordering::Release);
        let rc = cplug_sdk::retransform_class(t.name);
        eprintln!("[crussty-plugin] mobs_offgrid: {} armed, retransform rc={rc}", t.name);
    });
}
