...
/// TASK-461-69 (poi456-5): `cmp456_poi_p31snap` — STRICT eq (перенос P31+P32
/// sidecar на POI-носитель cmp456_poi, гипотеза poi-inside ≈ chunkmono).
pub fn enabled() -> bool {
    if std::env::var("CRUSSTY_LEVER_FLAG")
        .map(|v| {
            let v = v.trim();
            v == "cmp456_chunkmono_p31snap" || v == "cmp456_poi_p31snap"
        })
        .unwrap_or(false)
    {
        return true;
