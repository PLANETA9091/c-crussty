...
/**
 * OFF-THREAD STAGE (TASK-401-D, vector offthread — lever cmp401_offthread).
 *
 * Off-thread preparatory stage для мобового push-лейна. Порт семантики апстримного
 ...
 *     + floor-координаты для prep-окна;
 ...
 *   - перечисление кандидатов ВЫНЕСЕНО ИЗ ТИКА: один фоновый prep-поток
 *     непрерывно готовит per-id списки кандидатов 5³-окна
 *     (floor(center)±2 — надмножество любого padded-окна запроса при гейте
 *     half-extent ≤ 1.0) в фиксированную prepared-таблицу; в тике — ОДИН
 *     нативный stageQuery(id, lid, box, out): co-version валидность (ready +
 *     prep-cell == текущая ячейка грида + lid + окно запроса ⊆ prepared-
 *     окна) → только memcpy подготовленных id; любой stale/mismatch →
 *     ERR_RANGE → точный ванильный fill на этот вызов (parity by
 *     construction; статичные популяции хитуют, движители фейлятся в
 *     ваниль). Базовый грид — шардированный seqlock 1.0-ячеек (64 шарда
 *     mix64&63, v1->данные->v2 Acquire, писатель под глобальным mutex,
 *     фикс. 16384 слота/шард — pattern-B src/mobs_offgrid.rs);
 ...
 *
 * СВЕЖЕСТЬ: self-апсерт в момент запроса кладёт в грид end-of-move позицию
 * self; co-version гейт (prep cell == текущая ячейка) пропускает prepared-
 * результат только если self НЕ пересекал границу ячейки с момента prep —
 * его окно запроса целиком внутри prepared 5³-окна. Занос соседа в окно
 * после prep = недобор-кандидат того же documented-delta класса, что у
 * armed round-400-j-mobpush (излишки убиваются точной ре-валидацией).
 *
 * FAIL-CLOSED: ENABLED (env == "cmp401_offthread") && nativeOk (mobProbe magic)
 ...
 */
public final class MobStageOps {

    private static boolean leverEnabled() {
        String f = System.getenv("CRUSSTY_LEVER_FLAG");
        return f != null && f.trim().equals("cmp401_offthread");
    }

    private static final boolean ENABLED = leverEnabled();

    private static final int PROBE_MAGIC = 0x4F53; // "OS" (offstage)
    ...
    // ---- natives (impl: src/mobs_offgrid.rs, RegisterNatives после define) ----
    ...
    private static native int stageQuery(int id, int lid, double qx0, double qy0,
            double qz0, double qx1, double qy1, double qz1, int[] out);
    ...
    private static final ThreadLocal<ArrayList<Entity>[]> RING =
            ThreadLocal.withInitial(MobStageOps::newRing);
    ...
    private static boolean probeOnce() {
        ...
        synchronized (MobStageOps.class) {
            if (nativeOk) {
                return true;
            }
            try {
    ...
    /**
     * Замена сайта Level.getPushableEntities в LivingEntity.pushEntities (off-thread stage serve)
    ...
    public static List<Entity> pushables(Level level, Entity entity, AABB box) {
        ...
        int[] idBox = idMap.get(entity);
        if (idBox == null) {
            return vanillaFill(level, entity, box); // grow-гонка idMap — per-call vanilla
        }
        int[] out = SCRATCH.get();
        int n = stageQuery(idBox[0], lid,
                box.minX, box.minY, box.minZ, box.maxX, box.maxY, box.maxZ, out);
        if (n < 0) {
            if (n <= OVERFLOW_MAX) {
                out = new int[(-n) * 4];
                SCRATCH.set(out);
                n = stageQuery(idBox[0], lid,
                        box.minX, box.minY, box.minZ, box.maxX, box.maxY, box.maxZ, out);
            }
            if (n < 0) {
                if (n == ERR_STRUCT) {
    ...
    private static boolean upsertSelf(Entity e) {
        ...
        if (Math.max(hw, hh) > 1.0D) {
            ...
            LOG.warning("[crussty-plugin] cmp401_offthread: oversized bounding radius "
    ...
    private static void maybeSweep() {
        ...
        try {
            ...
            LOG.warning("[crussty-plugin] cmp401_offthread: sweep failed: " + t);
        } finally {
            SWEEPING.set(false);
        }
    }
