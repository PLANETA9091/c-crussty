#!/usr/bin/env python3
"""dispatch_451c.py — TASK-451-C (chunk-pipeline cycle-3, закон 3) — batch-3
density-ноги сертифицированного носителя cmp450_chunk (код = 201d9d66,
merged-гейты: blobs ALL IN SYNC, cargo 317/0 — RESEARCH-451-C §2(г)).
...
LEGS = [
    ("chunk-6", "round-451c-chunk-6", "round-451c-chunk", "cmp450_chunk"),
    ("chunk-7", "round-451c-chunk-7", "round-451c-chunk", "cmp450_chunk"),
    ("chunk-8", "round-451c-chunk-8", "round-451c-chunk", "cmp450_chunk"),
]

EXPECTED_SHA = {"round-451c-chunk": "201d9d66"}
...
def main():
    ...
    if "--help" in args or "-h" in args:
        print("usage: dispatch_451c.py [--dry-run] [--no-batch] [--leg name:branch:base:lever ...]")
    ...
    print("=== TASK-451-C DISPATCH (chunk-pipeline cycle-3, cmp450_chunk density batch-3) ===", flush=True)
    ...
    for base in sorted(bases):
        ...
        # ancestry-only canon (x440c step-3c) — pin must be an ANCESTOR.
    ...
    for leg, branch, base, lever in legs:
        ...
        time.sleep(4)
    print(f"=== TASK-451-C BATCH COMPLETE: {len(legs)} диспатчей ===", flush=True)
