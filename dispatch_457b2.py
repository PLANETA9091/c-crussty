#!/usr/bin/env python3
"""dispatch_457b2.py — TASK-457-B2 (RESTART after infra-cut): re-roll of the
BAND-DISCARD slot poi457-8 (+13.5@10425941, idx out of band 6.0-9.5M — not a
verdict, re-roll <=2 allowed). Legs poi457-10/11 (+12 reserve) on branches
round-456b-poi-10/11/12 created EXACTLY at fix commit
5ecd841a128aa97c633e8b005479c5dcf09781aa via API git/refs POST.

Canonical inputs identical to dispatch_457b.py (canon tick-456/457).
travel_diet / fluid_dirty_ledger are NOT sent (canon).

Usage: dispatch_457b2.py [--dry-run] [--no-batch] [--leg NAME]
"""
...
BATCH = [
    ("poi457-10", "round-456b-poi-10", "round-456b-poi", "cmp456_poi"),
    ("poi457-11", "round-456b-poi-11", "round-456b-poi", "cmp456_poi"),
    ("poi457-12", "round-456b-poi-12", "round-456b-poi", "cmp456_poi"),
]
...
def ensure_branch_exact(tok, branch):
    """Create branch at BASE_SHA via git/refs POST if missing; HARD-fail if drifted."""
    try:
...
def main():
    ...
    if "--help" in args or "-h" in args:
        print("usage: dispatch_457b2.py [--dry-run] [--no-batch] [--leg NAME]")
        raise SystemExit(0)
    ...
    print(f"=== TASK-457-B2 POI RE-ROLL DISPATCH (pin {BASE_SHA[:8]}, lever={LEVER}) ===", flush=True)
    ...
    for leg, branch, _, lever in batch:
        ...
        time.sleep(4)
    print(f"=== TASK-457-B2 BATCH COMPLETE: {len(batch)} dispatches ===", flush=True)
